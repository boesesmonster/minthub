do
  ...
  BringConnections = {}
  _B = true -- bring mobs enabled by default
  BossList = {}
  MaterialList = {}
  NPCList = {}
  shouldTween = false
...
-- combat defaults (WindUI may not fire default callbacks)
_G.Seriality = true
_G.ChooseWP = "Melee"
_B = true
...
CombatCore.Kill = function(model,Succes)
  ...
  Avocado_PullMobs(model)
  if not _G.SelectWeapon then
    for _,t in pairs(plr.Backpack:GetChildren()) do
      if t:IsA("Tool") and t.ToolTip == (_G.ChooseWP or "Melee") then
        _G.SelectWeapon = t.Name
        break
      end
    end
  end
  ...
  if not Equipped then
    for _,t in pairs(plr.Backpack:GetChildren()) do
      if t:IsA("Tool") and (t.Name == _G.SelectWeapon or t.ToolTip == (_G.ChooseWP or "Melee")) then
        Avocado_EquipTool(t.Name)
        Equipped = game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChildOfClass("Tool")
        break
      end
    end
  end
            ...
            if hum and root and hum.Health > 0 then
                ...
                if distance <= bringRange then
                    pcall(function()
                        ...
                        hum.WalkSpeed = 0
                        hum.JumpPower = 0
                        if hum.JumpHeight then hum.JumpHeight = 0 end
                        root.CanCollide = false
                        root.AssemblyLinearVelocity = Vector3.zero
                        root.AssemblyAngularVelocity = Vector3.zero

                        local bv = root:FindFirstChild("AvocadoBV")
                        if not bv then
                        ...
                        local bg = root:FindFirstChild("AvocadoBG")
                        if not bg then
                        ...
                        -- stack near main target (small offset so they overlap)
...
Hop = function() end -- server hop disabled
...
Tabs.Status:Section({ Title = "Overview" })


Tabs.Status:Section({ Title = "Live Status" })
...
spawn(function()
    while task.wait(0.5) do
        pcall(function()
            local tip = _G.ChooseWP or "Melee"
...
Tabs.AutoFarm:Section({ Title = "Level Grind" })
...
Tabs.AutoFarm:Section({ Title = "Berry Collect" })
...
Tabs.AutoFarm:Section({ Title = "Target Mobs" })
if World1 then
    Tabs.AutoFarm:Dropdown({
...
Tabs.AutoFarm:Section({ Title = "Island Sweep" })
...
Tabs.AutoFarm:Section({ Title = "Elite Targets" })
...
local function HopServer() end -- server hop disabled
...
Tabs.AutoFarm:Section({ Title = "Indra" })
...
Tabs.AutoFarm:Section({ Title = "Cake Event" })
...
Tabs.AutoFarm:Section({ Title = "Bone Grind" })
...
Tabs.AutoFarm:Section({ Title = "Sky Tyrant" })
...
Tabs.AutoFarm:Section({ Title = "Materials" })
...
Tabs.AutoFarm:Section({ Title = "Boss Targets" })
...
Tabs.AutoFarm:Section({ Title = "Mastery Grind" })
...
Tabs.Config:Section({ Title = "General Config" })
...
Callback = function(Value)
  ...
  FastAttackModule.Enabled = Value and true or false
end})
...
Tabs.Missions:Toggle({
    Title = "Single Quest",
    Flag = "Toggle_Single_Quest",
    Value = false,
    Callback = function(Value)
        _G.SingleQuest = Value
        if Value then _G.DoubleQuest = false; _G.TripleQuest = false end
    end
})
Tabs.Missions:Toggle({
    Title = "Double Quest",
    Flag = "Toggle_Double_Quest",
    Value = false,
    Callback = function(Value)
        _G.DoubleQuest = Value
        if Value then _G.SingleQuest = false; _G.TripleQuest = false end
    end
})
Tabs.Missions:Toggle({
    Title = "Triple Quest",
    Flag = "Toggle_Triple_Quest",
    Value = false,
    Callback = function(Value)
        _G.TripleQuest = Value
        if Value then _G.SingleQuest = false; _G.DoubleQuest = false end
    end
})
...
-- GetQuestDracoLevel removed

-- removed draco v3/v4/trial feature
...
Tabs.Market:Section({ Title = "Market Options" })
Tabs.Market:Button({
Title = "Buy Buso", 
Desc = "",
Callback = function()
...
Tabs.Market:Section({ Title = "Fight Styles" })
Tabs.Market:Button({
Title = "Buy Black Leg", 
Desc = "",
Callback = function()
...
Tabs.Market:Section({ Title = "Fragments" })
Tabs.Market:Button({
    ...
    pcall(function()      
      ...
      if not Pretool then
        ...
        for _, t in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
          if t:IsA("Tool") and (t.ToolTip == tip or t.Name == _G.SelectWeapon) then
            pcall(function() char.Humanoid:EquipTool(t) end)
            Pretool = char:FindFirstChildOfClass("Tool")
            break
          end
        end
      end
...
function FastAttackModule.ExecuteFastAttack()
    ...
    if not tool then
        ...
        for _, t in pairs(LocalPlayer.Backpack:GetChildren()) do
            if t:IsA("Tool") and (t.ToolTip == tip or t.Name == _G.SelectWeapon) then
                pcall(function() character.Humanoid:EquipTool(t) end)
    ...
    local attackRemote = Net:FindFirstChild("RE/RegisterAttack") or Net["RE/RegisterAttack"]
    local hitRemote = Net:FindFirstChild("RE/RegisterHit") or Net["RE/RegisterHit"]
    ...
    -- fallback click (helps on some executors / melee)
    pcall(function()
        local vu = game:GetService("VirtualUser")
...
function HitRegistrationModule.Execute()
    ...
    if #hitTargets > 0 and tool and (
            tool:GetAttribute("WeaponType") == "Melee"
            or tool:GetAttribute("WeaponType") == "Sword"
            or tool.ToolTip == "Melee"
