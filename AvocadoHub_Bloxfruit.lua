...
Avocado_EquipTool = function(text)
  if not text then return end
  if plr.Backpack:FindFirstChild(text) then
	plr.Character.Humanoid:EquipTool(plr.Backpack:FindFirstChild(text))
  end
end
Avocado_EquipByType = function(weapon)
  for __in, v in pairs(plr.Backpack:GetChildren()) do
    if v:IsA("Tool") then
      if v.ToolTip == weapon then Avocado_EquipTool(v.Name) end
    end
  end
end
local CombatCore = {}
CombatCore.__index = Attack
CombatCore.Alive = function(model) if not model then return end local Humanoid = model:FindFirstChild("Humanoid") return Humanoid and Humanoid.Health > 0 end
CombatCore.Pos = function(model,dist) return (Root.Position - mode.Position).Magnitude <= dist end
CombatCore.Dist = function(model,dist) return (Root.Position - model:FindFirstChild("HumanoidRootPart").Position).Magnitude <= dist end
CombatCore.DistH = function(model,dist) return (Root.Position - model:FindFirstChild("HumanoidRootPart").Position).Magnitude > dist end
CombatCore.Kill = function(model,Succes)
  if model and Succes then
  if not model:GetAttribute("Locked") then model:SetAttribute("Locked",model.HumanoidRootPart.CFrame) end
  PosMon = model:GetAttribute("Locked").Position
  Avocado_PullMobs()
  Avocado_EquipTool(_G.SelectWeapon)
...
CombatCore.Kill2 = function(model,Succes)
  if model and Succes then
  if not model:GetAttribute("Locked") then model:SetAttribute("Locked",model.HumanoidRootPart.CFrame) end
  PosMon = model:GetAttribute("Locked").Position
  Avocado_PullMobs()
...
CombatCore.KillSea = function(model,Succes)
  if model and Succes then
  if not model:GetAttribute("Locked") then model:SetAttribute("Locked",model.HumanoidRootPart.CFrame) end
  PosMon = model:GetAttribute("Locked").Position
  Avocado_PullMobs()
...
CombatCore.Sword = function(model,Succes)
  if model and Succes then
  if not model:GetAttribute("Locked") then model:SetAttribute("Locked",model.HumanoidRootPart.CFrame) end
  PosMon = model:GetAttribute("Locked").Position
  Avocado_PullMobs()
  Avocado_EquipByType("Sword")
  _tp(model.HumanoidRootPart.CFrame * CFrame.new(0,30,0))
  if RandomCFrame then wait(0.1)_tp(model.HumanoidRootPart.CFrame * CFrame.new(0, 30, 25)) wait(0.1)_tp(model.HumanoidRootPart.CFrame * CFrame.new(25, 30, 0)) wait(0.1)_tp(model.HumanoidRootPart.CFrame * CFrame.new(-25, 30 ,0)) wait(0.1)_tp(model.HumanoidRootPart.CFrame * CFrame.new(0, 30, 25)) wait(0.1)_tp(model.HumanoidRootPart.CFrame * CFrame.new(-25, 30, 0))end
  end
end
CombatCore.Mas = function(model,Succes)
  if model and Succes then
  if not model:GetAttribute("Locked") then model:SetAttribute("Locked",model.HumanoidRootPart.CFrame) end
  PosMon = model:GetAttribute("Locked").Position
  Avocado_PullMobs()
    if model.Humanoid.Health <= HealthM then
      ...
      Avocado_CastSkill("Blox Fruit","Z")
      Avocado_CastSkill("Blox Fruit","X")
      Avocado_CastSkill("Blox Fruit","C")
    else
      Avocado_EquipByType("Melee")
      _tp(model.HumanoidRootPart.CFrame * CFrame.new(0,30,0))
    end
  end
end
CombatCore.Masgun = function(model,Succes)
  if model and Succes then
  if not model:GetAttribute("Locked") then model:SetAttribute("Locked",model.HumanoidRootPart.CFrame) end
  PosMon = model:GetAttribute("Locked").Position
  Avocado_PullMobs()
    if model.Humanoid.Health <= HealthM then
      ...
      Avocado_CastSkill("Gun","Z")
      Avocado_CastSkill("Gun","X")
    else
      Avocado_EquipByType("Melee")
      _tp(model.HumanoidRootPart.CFrame * CFrame.new(0,30,0))
    end
...
Avocado_AddStat = function(Num, value)
  if Num == "Melee" then
...
Avocado_PullMobs = function(Mon)
    if not _B then return end
    local maxBring = 5 -- bring up to 5 mobs total
    ...
    pcall(function()
        ...
        -- collect same-name living mobs, sort by distance to main target
        local list = {}
        for _, v in ipairs(workspace.Enemies:GetChildren()) do
            local alive, root, hum = Mobs(v)
            if alive and v.Name == monName then
                local d = (root.Position - targetPos).Magnitude
                if d <= 3000 then
                    table.insert(list, {model = v, root = root, hum = hum, dist = d})
                end
            end
        end
        table.sort(list, function(a, b) return a.dist < b.dist end)

        -- keep closest maxBring (includes main)
        for i = 1, math.min(maxBring, #list) do
            local e = list[i]
            ...
            pcall(function()
                ...
                hum.WalkSpeed = 0
                hum.JumpPower = 0
                -- stack near main with tiny offset so they overlap for AOE hits
                local ox = ((i - 1) % 3) * 0.6 - 0.6
                local oz = (math.floor((i - 1) / 3)) * 0.6
                root.CFrame = CFrame.new(targetPos + Vector3.new(ox, 0, oz))
            end)
        end

        if Mon:FindFirstChild("HumanoidRootPart") then
            Mon.HumanoidRootPart.CanCollide = false
            if Mon:FindFirstChild("Humanoid") then
...
Avocado_CastSkill = function(weapon, skill)
  ...
  elseif weapon == "Blox Fruit" then
    Avocado_EquipByType("Blox Fruit")
    if skill == "Z" then
      vim1:SendKeyEvent(true, "Z", false, game);
      vim1:SendKeyEvent(false, "Z", false, game);
    elseif skill == "X" then
...
CheckF = function()
  if Avocado_HasTool("Dragon-Dragon") or Avocado_HasTool("Gas-Gas") or Avocado_HasTool("Yeti-Yeti") or Avocado_HasTool("Kitsune-Kitsune") or Avocado_HasTool("T-Rex-T-Rex") then return true end
end
CheckBoat = function()
...
CheckShark = function()
  for _,v in pairs(workspace.Enemies:GetChildren()) do
    if v.Name == "Shark" and CombatCore.Alive(v) then
      return true    
end;
  end;
  return false
...
CheckTerrorShark = function()
  for _,v in pairs(workspace.Enemies:GetChildren()) do
    if v.Name == "Terrorshark" and CombatCore.Alive(v) then
      return true    
end;
  end;
  return false
...
CheckPiranha = function()
  for _,v in pairs(workspace.Enemies:GetChildren()) do
    if v.Name == "Piranha" and CombatCore.Alive(v) then
      return true    
end;
  end;
  return false
...
CheckFishCrew = function()
  for _,v in pairs(workspace.Enemies:GetChildren()) do
    if (v.Name == "Fish Crew Member" or v.Name == "Haunted Crew Member") and CombatCore.Alive(v) then
      return true    
end;
  end;
  return false
...
CheckHauntedCrew = function()
  for _,v in pairs(workspace.Enemies:GetChildren()) do
    if (v.Name == "Haunted Crew Member") and CombatCore.Alive(v) then
      return true    
end;
  end;
  return false
...
Avocado_HasLeviathan = function()
  if workspace.SeaBeasts:FindFirstChild("Leviathan") then
    return true  
end;
  return false
end;
Avocado_StoreFruits = function()
...
Avocado_CollectFruits = function(Succes)
  if Succes then
...
Avocado_DropFruits = function()
  for _,v3 in next, plr.Backpack:GetChildren() do
    if string.find(v3.Name, "Fruit") then
      Avocado_EquipTool(v3.Name) wait(.1)
      if plr.PlayerGui.Main.Dialogue.Visible == true then plr.PlayerGui.Main.Dialogue.Visible = false end Avocado_EquipTool(v3.Name) plr.Character:FindFirstChild(v3.Name).EatRemote:InvokeServer("Drop")
    end
  end
  for a,b2 in pairs(plr.Character:GetChildren()) do
    if string.find(b2.Name, "Fruit") then Avocado_EquipTool(b2.Name) wait(.1)
    if plr.PlayerGui.Main.Dialogue.Visible == true then plr.PlayerGui.Main.Dialogue.Visible = false end Avocado_EquipTool(b2.Name) plr.Character:FindFirstChild(b2.Name).EatRemote:InvokeServer("Drop")
    end
  end
end
Avocado_HasTool = function(v)
  return plr.Backpack:FindFirstChild(v) or plr.Character:FindFirstChild(v)
end
Avocado_HasInventory = function(Name)
...
Avocado_MaterialCount = function(Name)
...
Avocado_HasWeapon = function(nametool)
...
Avocado_BossQuestData = function()
				if World1 then
			...
			Avocado_BossQuestPack = function()
				local Neta = Avocado_BossQuestData()
				return {
					[0] = _G.FindBoss,
					[1] = bMon,
					[2] = Qdata,
...
Avocado_QuestNPCAnchor = function(npcName)
...
Avocado_QuestAnchor = function()
    if GuideModule and GuideModule.Data and GuideModule.Data.LastClosestNPC then
        return Avocado_QuestNPCAnchor(GuideModule.Data.LastClosestNPC)
    end
    return nil
end
...
Avocado_LevelQuestPack = function()
    local questData = Avocado_ResolveQuest()
    return {
        [1] = questData.Mob,           
        [2] = questData.ID,             
...
WindUI:AddTheme({
    Name = "avocado",
    Accent = Color3.fromRGB(86, 166, 66),
    Background = Color3.fromRGB(14, 22, 12),
    Outline = Color3.fromRGB(50, 90, 40),
    Text = Color3.fromRGB(235, 245, 230),
    Placeholder = Color3.fromRGB(140, 170, 130),
    Button = Color3.fromRGB(40, 70, 35),
    Icon = Color3.fromRGB(160, 210, 140),
    Toggle = Color3.fromRGB(86, 166, 66),
    Slider = Color3.fromRGB(86, 166, 66),
    Checkbox = Color3.fromRGB(86, 166, 66),
    Dialog = Color3.fromRGB(18, 28, 16),
})
WindUI:SetTheme("avocado")
...
local Window = WindUI:CreateWindow({
    ...
    Folder = "AvocadoHub",
    Theme = "avocado",
    Size = IsMobile and UDim2.fromOffset(480, 490) or UDim2.fromOffset(580, 460),
    Transparent = true,
    Resizable = true,
    SideBarWidth = IsMobile and 130 or 180,
...
local Tabs = {
    ...
    Missions = Window:Tab({ Title = "Missions", Icon = "scroll-text" }),
    Angler = Window:Tab({ Title = "Angler", Icon = "fish" }),
    Ocean = Window:Tab({ Title = "Ocean", Icon = "waves" }),
    Mirage = Window:Tab({ Title = "Mirage", Icon = "moon" }),
    Volcano = Window:Tab({ Title = "Volcano", Icon = "flame" }),
    Visuals = Window:Tab({ Title = "Visuals", Icon = "eye" }),
    Fruits = Window:Tab({ Title = "Fruits", Icon = "zap" }),
    Player = Window:Tab({ Title = "Player", Icon = "user" }),
    Warp = Window:Tab({ Title = "Warp", Icon = "map-pin" }),
    Market = Window:Tab({ Title = "Market", Icon = "shopping-bag" }),
    Extra = Window:Tab({ Title = "Extra", Icon = "layers" })
}

Tabs.Status:Section({ Title = "Information" })


Tabs.Status:Section({ Title = "Status Server" })

local TimeZone = Tabs.Status:Paragraph({ Title = "Time Zone", Desc = "" })
...
local GameTime = Tabs.Status:Paragraph({ Title = "Game Time", Desc = "" })
...
local MirageCheck = Tabs.Status:Paragraph({ Title = "Mirage Island", Desc = "Status: " })
...
local KitsuneCheck = Tabs.Status:Paragraph({ Title = "Kitsune Island", Desc = "Status: " })
...
local PrehistoricCheck = Tabs.Status:Paragraph({ Title = "Prehistoric Island", Desc = "Status: " })
...
local FrozenCheck = Tabs.Status:Paragraph({ Title = "Frozen Dimension", Desc = "Status: " })
...
local CakePrinceStatus = Tabs.Status:Paragraph({ Title = "Cake Prince", Desc = "" })
...
local RipIndraCheck = Tabs.Status:Paragraph({ Title = "Rip Indra", Desc = "Status: " })
...
local DoughKingCheck = Tabs.Status:Paragraph({ Title = "Dough King", Desc = "Status: " })
...
local FullMoonCheck = Tabs.Status:Paragraph({ Title = "Full Moon", Desc = "" })
...
local LegendarySwordCheck = Tabs.Status:Paragraph({ Title = "Legendary Sword", Desc = "Status: " })
...
local BoneCount = Tabs.Status:Paragraph({ Title = "Bone", Desc = "" })
...
WeaponDropdown = Tabs.AutoFarm:Dropdown({
    Flag = "Dropdown_Select_Weapon",
    Title = "Select Weapon",
...
Tabs.AutoFarm:Section({ Title = "Farming" })

FarmLevel = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Farm_Level",
        ...
        if _G.Level then
            pcall(function()
                ...
                elseif inSub or level < 2600 then
                    ...
                    local questData = Avocado_LevelQuestPack()
                    ...
                    for _, v in pairs(workspace.Enemies:GetChildren()) do
                        if v.Name == enemyName and CombatCore.Alive(v) then
                            foundMob = true
                            repeat
                                task.wait(Sec)
                                _tp(v.HumanoidRootPart.CFrame * CFrame.new(0,20,0))
                                CombatCore.Kill(v, _G.Level)
...
ClosetMons = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Farm_Nearest",
        ...
        for i,v in pairs(workspace.Enemies:GetChildren()) do
          if v:FindFirstChild("Humanoid") or v:FindFirstChild("HumanoidRootPart") then
            if v.Humanoid.Health > 0 then
              repeat wait() CombatCore.Kill(v,_G.AutoFarmNear) until not _G.AutoFarmNear or not v.Parent or v.Humanoid.Health <= 0
            end
          end
        end
      end
...
FactoryRaids = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Factory_Raid",
  ...
  while wait(Sec) do
    pcall(function()
      if _G.AutoFactory then
        local v = Avocado_FindEnemy("Core")
        if v then
          repeat wait()
            Avocado_EquipTool(_G.SelectWeapon)
...
CastleRaids = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Pirate_Raid",
            ...
            if v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
              if v.Name then
                if (v.HumanoidRootPart.Position - Root.Position).Magnitude <= 2000 then
                  repeat wait() CombatCore.Kill(v,_G.AutoRaidCastle) until not _G.AutoRaidCastle or not v.Parent or v.Humanoid.Health <= 0 or not workspace.Enemies:FindFirstChild(v.Name)
                end
              end
            end
          end
...
Ecto = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Farm_Ectoplasm",
  ...
  while wait(Sec) do
    pcall(function()
      if _G.AutoEctoplasm then
        ...
        local v = Avocado_FindEnemy(EctoTable)
		if CombatCore.Alive(v) then
		  repeat wait() CombatCore.Kill(v, _G.AutoEctoplasm)until not _G.AutoEctoplasm or not v.Parent or v.Humanoid.Health <= 0		        
	    else
...
Tabs.AutoFarm:Section({ Title = "Chest" })

ChestTW = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Farm_Chest",
...
ChestBP = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Chest_Bypass",
...
StopI = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Stop_Items",
Title = "Stop Items", 
Desc = "", 
Value = true,
Callback = function(Value)
...
spawn(function()
    while wait(0.2) do
        if _G.StopWhenChalice and (_G.AutoFarmChest or _G.AutoChestBP) then
            pcall(function()
                if Avocado_HasTool("God's Chalice") or Avocado_HasTool("Sweet Chalice") or Avocado_HasTool("Fist of Darkness") then
...
Tabs.AutoFarm:Section({ Title = "Collect Berry" })

Berry = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Farm_Berry",
...
Tabs.AutoFarm:Section({ Title = "Farm Mob" })
if World1 then
    Tabs.AutoFarm:Dropdown({
    Flag = "Dropdown_Select_Mob",
        Title = "Select Mob",
        Value = Bandit,
        Values = {
                        ...
                        if v.Name == getgenv().SelectMob then
                            ...
                            and v.Humanoid.Health > 0 then                                
                                repeat
                                    ...
                                    CombatCore.Kill(v,_G.AutoKillMob)
...
Tabs.AutoFarm:Section({ Title = "Farm All Island" })
...
Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Farm_All_Island",
...
Tabs.AutoFarm:Section({ Title = "Farm Elite Hunter" })

local Process = Tabs.AutoFarm:Paragraph({ Title = "Elites Process", Desc = "" })
spawn(function()
    while wait(Sec) do
        pcall(function()    
...
local EliteHunter = Tabs.AutoFarm:Paragraph({ Title = "Elite Spawn", Desc = "Status: " })
spawn(function()
...
EliteQ = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Farm_Elite",
                ...
                if not questGui.Visible then
                    ...
                    if eliteName then
                        ...
                        for _, v in pairs(Enemies:GetChildren()) do
                            if v.Name == eliteName and CombatCore.Alive(v) then
                                boss = v
                                break
                            end
                        end
                        ...
                        if boss and boss:FindFirstChild("HumanoidRootPart") then
                            ...
                            repeat
                                wait()
                                CombatCore.Kill(boss, _G.FarmEliteHunt)
				...
				if not questGui.Visible then
					...
					if eliteName then
						...
						if boss and boss:FindFirstChild("HumanoidRootPart") then
							...
							repeat
								wait()
								CombatCore.Kill(boss, _G.FarmEliteH)
...
Tabs.AutoFarm:Section({ Title = "Farm Rip Indra" })

Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Attack_Rip_Indra",
  ...
  while wait(Sec) do
    pcall(function()
      if _G.AutoRipIngay then
        local v = Avocado_FindEnemy("rip_indra")
	    if not Avocado_HasWeapon("Dark Dagger") or not Avocado_HasInventory("Valkyrie") and v then
	      repeat wait() CombatCore.Kill(v,_G.AutoRipIngay)until not _G.AutoRipIngay or not v.Parent or v.Humanoid.Health <= 0
        else
...
Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Unlocked_Haki",
...
Tabs.AutoFarm:Section({ Title = "Farming Cake" })
local MobKilled = Tabs.AutoFarm:Paragraph({ Title = "Cake Princes", Desc = "" })
spawn(function()
    while wait(0.2) do
        pcall(function()
...
Cake = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Farm_Cake_Prince",
        ...
        if _G.Auto_Cake_Prince and not _G.AutoRaidCastle then
            pcall(function()
                ...
                if bigMirror and (bigMirror.Other.Transparency == 0 or enemies:FindFirstChild("Cake Prince")) then
                    local boss = Avocado_FindEnemy("Cake Prince")
                    if boss then
                        repeat task.wait()
                            CombatCore.Kill2(boss, _G.Auto_Cake_Prince)
                    ...
                    local mob = Avocado_FindEnemy(CakeMobs)
                    if mob then
                        repeat task.wait()
                            CombatCore.Kill(mob, _G.Auto_Cake_Prince)
...
CakeQ = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Accept_Quests",
Title = "Accept Quests", 
Desc = "", 
Value = false,
Callback = function(Value)
...
CakeSM = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Summon_Cake_Prince",
...
Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Dough_King_Fully",
        ...
        if _G.AutoDoughKing then
            pcall(function()
                if not workspace.Map.CakeLoaf:FindFirstChild("RedDoor") then
                    if Avocado_HasTool("Red Key") then
                ...
                elseif workspace.Map.CakeLoaf:FindFirstChild("RedDoor") then
                    if Avocado_HasTool("Red Key") then
                        ...
                        Avocado_EquipTool("Red Key")
                    end
                elseif Avocado_FindEnemy("Dough King") then
                    local v = Avocado_FindEnemy("Dough King")
                    if v then
                        repeat
                            task.wait()
                            CombatCore.Kill(v, _G.AutoDoughKing)
                        until not _G.AutoDoughKing or not v.Parent or v.Humanoid.Health <= 0
                    else
                        _tp(CFrame.new(-1943.676513671875, 251.5095672607422, -12337.880859375))
                    end
                end
                if Avocado_HasTool("Sweet Chalice") then
                    replicated.Remotes.CommF_:InvokeServer("CakePrinceSpawner", true)
                    _G.AutoAttackDoughKing = true
                else
                    _G.AutoAttackDoughKing = false
                end
                if Avocado_HasTool("God's Chalice") and Avocado_MaterialCount("Conjured Cocoa") >= 10 then
                ...
                if Avocado_MaterialCount("Conjured Cocoa") <= 10 then
                    local v = Avocado_FindEnemy{"Cocoa Warrior", "Chocolate Bar Battler"}
                    if v then
                        repeat
                            task.wait()
                            CombatCore.Kill(v, _G.AutoDoughKing)
...
Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Farm_Dough_King",
        ...
        if _G.AutoAttackDoughKing then
            pcall(function()
                ...
                if v then
                    repeat 
                        task.wait()
                        CombatCore.Kill(v,_G.AutoAttackDoughKing)
        ...
        if _G.AutoHop_Dough then
            pcall(function()
                ...
                if v then
                    ...
                    repeat 
                        task.wait()
                        CombatCore.Kill(v, _G.AutoHop_Dough)
                    ...
                    local checkAgain = Avocado_FindEnemy("Dough King")
...
Tabs.AutoFarm:Section({ Title = "Farming Bone" })

local CheckingBone = Tabs.AutoFarm:Paragraph({ Title = "Bones", Desc = "" })
spawn(function()
    while wait(0.2) do
        pcall(function()
            CheckingBone:SetDesc("Bones : " .. Avocado_MaterialCount("Bones"))
        end)
    end
end)

Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Farm_Bone",
...
spawn(function()
    ...
    while wait(0.5) do
        ...
        pcall(function()
            ...
            local bone = Avocado_FindEnemy(BonesTable)
...
BoneQ = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Accept_Quests_2",
Title = "Accept Quests", 
Desc = "", 
Value = false,
Callback = function(Value)
...
spawn(function()
  while wait(Sec) do
    if _G.AutoHytHallow then
      pcall(function()
        local v = Avocado_FindEnemy("Soul Reaper")
	    if v then
          repeat task.wait() CombatCore.Kill(v,_G.AutoHytHallow) until v.Humanoid.Health <= 0 or _G.AutoHytHallow == false
        else
          if not Avocado_HasTool("Hallow Essence") then
            repeat task.wait(.1)replicated.Remotes.CommF_:InvokeServer("Bones","Buy",1,1)until _G.AutoHytHallow == false or Avocado_HasTool("Hallow Essence")
          else
		    ...
		    Avocado_EquipTool("Hallow Essence")
          end
        end
      end)
    end
...
RanBone = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Random_Bones",
...
Lucky = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Try_Luck_Gravestone",
...
Pray = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Pray_Gravestone",
...
Tabs.AutoFarm:Section({ Title = "Tyrant of the Skies" })

local TyrantStatus = Tabs.AutoFarm:Paragraph({ Title = "Boss Spawn", Desc = "" })
spawn(function()
    pcall(function()
        while wait(1) do
...
local EyeStatus = Tabs.AutoFarm:Paragraph({ Title = "Check Status Eyes", Desc = "" })
...
FarmTyrant = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Farm_Boss_TOTS",
            ...
            pcall(function()
                ...
                if boss and boss:FindFirstChild("Humanoid") and boss.Humanoid.Health > 0 then
                    repeat
                        ...
                        if Attack and CombatCore.Kill then
                            CombatCore.Kill(boss, _G.FarmTyrant)
                        end
                        wait()
                    ...
                    for _, mob in pairs(workspace.Enemies:GetChildren()) do
                        ...
                        if mob and mob.Name == mobName and mob:FindFirstChild("HumanoidRootPart") and mob:FindFirstChild("Humanoid") and mob.Humanoid.Health > 0 then
                            ...
                            repeat
                                if not _G.FarmTyrant then break end
                                if Attack and CombatCore.Kill then
                                    CombatCore.Kill(mob, _G.FarmTyrant)
                                end
                                wait()
...
FarmPhaBinh = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Summon_Boss",
...
Tabs.AutoFarm:Section({ Title = "Farm Material" })

Test = Tabs.AutoFarm:Dropdown({
    Flag = "Dropdown_Choose_Material",
...
Toggle = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Farm_Materials",
...
spawn(function()
  local function processEnemy(v, EnemyName)
    if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
      if v.Name == EnemyName then repeat wait() CombatCore.Kill(v,getgenv().AutoMaterial) until not getgenv().AutoMaterial or not v.Parent or v.Humanoid.Health <= 0 end
    end
  end
...
Tabs.AutoFarm:Section({ Title = "Farm Boss" })

		BossDropdown = Tabs.AutoFarm:Dropdown({
    Flag = "Dropdown_Select_Boss",
		Title = "Select Boss",
		Desc = "",
		Values = BossList,
		Callback = function(value)
...
FarmBoss = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Farm_Boss",
    ...
    Callback = function(value)
        ...
        spawn(function()
            while wait(Sec) do
                if _G.FarmBoss then
                    pcall(function()
                        local HasQuest = Avocado_BossQuestPack()[2] ~= nil and Avocado_BossQuestPack()[3] ~= nil
                        local QuestTitle = plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text

                       
                        if _G.AcceptQuestBoss and HasQuest then
                            if not string.find(QuestTitle, Avocado_BossQuestPack()[0]) then
                                replicated.Remotes.CommF_:InvokeServer("AbandonQuest")
                            end

                            if plr.PlayerGui.Main.Quest.Visible == false then
                                _tp(Avocado_BossQuestPack()[5])
                                if (Root.Position - Avocado_BossQuestPack()[5].Position).Magnitude <= 5 then
                                    replicated.Remotes.CommF_:InvokeServer("StartQuest", Avocado_BossQuestPack()[3], Avocado_BossQuestPack()[2])
                                end
                            elseif plr.PlayerGui.Main.Quest.Visible == true then
                                if workspace.Enemies:FindFirstChild(Avocado_BossQuestPack()[1]) then
                                    for i, v in pairs(workspace.Enemies:GetChildren()) do
                                        if CombatCore.Alive(v) and v.Name == Avocado_BossQuestPack()[1] then
                                            if string.find(QuestTitle, Avocado_BossQuestPack()[0]) then
                                                repeat
                                                    wait()
                                                    CombatCore.Kill(v, _G.FarmBoss)
                                    ...
                                    _tp(Avocado_BossQuestPack()[4])
                                    if replicated:FindFirstChild(Avocado_BossQuestPack()[1]) then
                                        _tp(replicated:FindFirstChild(Avocado_BossQuestPack()[1]).HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                    end
                                end
                            end
                        else
...
BossQ = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Accept_Quests_3",
    Title = "Accept Quests",
    Desc = "",
    Value = true,
    Callback = function(Value)
...
FarmAllBoss = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Farm_All_Boss",
            ...
            pcall(function()
                ...
                if nearestBoss and nearestBoss:FindFirstChild("HumanoidRootPart") then
                    ...
                    repeat
                        ...
                        if Attack and typeof(CombatCore.Kill) == "function" then
                            CombatCore.Kill(nearestBoss, true)
                        end
...
Tabs.AutoFarm:Section({ Title = "Farming Mastery" })
local posMastery = {"Cake","Bone"}
local Mastery_Config = Tabs.AutoFarm:Dropdown({
    Flag = "Dropdown_Choose_Island",
Title = "Choose Island",
		Desc = "",
		Values = posMastery,
		Value = Bone,
...
local MasteryFruits = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Mastery_Fruits",
...
local MasteryGun = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Mastery_Gun",
      ...
      pcall(function()
        if SelectIsland == "Cake" then
		  ...
		  if v then		      
		    ...
		    repeat wait()
		      MousePos = v.HumanoidRootPart.Position
		      CombatCore.Masgun(v,_G.FarmMastery_G)
...
local MasterySword = Tabs.AutoFarm:Toggle({
    Flag = "Toggle_Auto_Mastery_All_Sword",
            ...
            if type(v) == "table" then
              if v.Type == "Sword" then
                ...
                if tonumber(v.Mastery) >= 1 or tonumber(v.Mastery) <= 599 then
                  local v = Avocado_FindEnemy(mastery1)
                  if Avocado_HasTool(SwordName) then                    
		            if v then
                      repeat wait() CombatCore.Sword(v,_G.FarmMastery_S) until _G.FarmMastery_S == false or not v.Parent or v.Humanoid.Health <= 0		                  
		            else
		              _tp(CFrame.new(-1943.676513671875, 251.5095672607422, -12337.880859375)) 
		            end                    
                  else
              ...
              elseif tonumber(v.Mastery) >= 600 then
                if Avocado_HasTool(SwordName) then return nil else replicated.Remotes.CommF_:InvokeServer("LoadItem",SwordName) end       
              end
                break
              end
            end         
...
Initialize = Tabs.Config:Toggle({
    Flag = "Toggle_Fast_Attack",
Title = "Fast Attack", 
Desc = "", 
Value = true,
Callback = function(Value)
...
Bringmob = Tabs.Config:Toggle({
    Flag = "Toggle_Bring_Mobs",
Title = "Bring Mobs", 
Desc = "", 
Value = true,
Callback = function(Value)
...
Tabs.Config:Toggle({
    Flag = "Toggle_Auto_Set_Spawn_Point",
...
BusuAura = Tabs.Config:Toggle({
    Flag = "Toggle_Auto_Turn_on_Buso",
...
Tabs.Config:Toggle({
    Flag = "Toggle_Auto_Haki_Observation",
...
RaceV4Aura = Tabs.Config:Toggle({
    Flag = "Toggle_Auto_Turn_on_Race_V4",
  ...
  while wait(.2) do
    pcall(function()
      if _G.RaceClickAutov4 then
        ...
        if plr.Character:FindFirstChild("RaceEnergy").Value == 1 then Avocado_CastSkill("nil","Y") end
        end        
      end 
    end)
  end
...
RandomAround = Tabs.Config:Toggle({
    Flag = "Toggle_Auto_Turn_on_Spin_xyz",
...
SafeModes = Tabs.Config:Toggle({
    Flag = "Toggle_Safe_Mode",
Title = "Safe Mode", 
Desc = "", 
Value = false,
Callback = function(Value)
...
DisableHitVFX = Tabs.Config:Toggle({
    Flag = "Toggle_Remove_Hit_VFX",
    Title = "Remove Hit VFX",
    Desc = "Removes slash and sword visual effects for better visibility",
    Value = false,
    Callback = function(Value)
        _G.DestroyHit = Value
    end
...
RmvVFX = Tabs.Config:Toggle({
    Flag = "Toggle_Remove_Death_Respawned_VFX",
...
DisblesNotify = Tabs.Config:Toggle({
    Flag = "Toggle_Disable_Notify",
...
Tabs.Config:Toggle({
    Flag = "Toggle_Auto_Anti_Admin_Join_Server",
...
Tabs.Visuals:Section({ Title = "Stats Upgrade" })

StatusSelect = Tabs.Visuals:Slider({
    Flag = "Slider_Stats_Value",
    Value = { Min = 0, Max = 1000, Default = 10 },
Title = "Stats Value",
Desc = "",
Callback = function(Value)
  pSats = Value
...
StatsUpg = Tabs.Visuals:Toggle({
    Flag = "Toggle_Auto_Melee",
Title = "Auto Melee", 
Desc = "", 
Value = false,
Callback = function(Value)
...
spawn(function()
  while wait(Sec) do
    pcall(function()
    if _G.Auto_Melee then Avocado_AddStat("Melee",pSats) end
    end)
  end
end)

StatsUpg = Tabs.Visuals:Toggle({
    Flag = "Toggle_Auto_Swords",
Title = "Auto Swords", 
Desc = "", 
Value = false,
Callback = function(Value)
...
spawn(function()
  while wait(Sec) do
    pcall(function()
    if _G.Auto_Sword then Avocado_AddStat("Sword",pSats) end
    end)
  end
end)
StatsUpg = Tabs.Visuals:Toggle({
    Flag = "Toggle_Auto_Gun",
Title = "Auto Gun", 
Desc = "", 
Value = false,
Callback = function(Value)
...
spawn(function()
  while wait(Sec) do
    pcall(function()
    if _G.Auto_Gun then Avocado_AddStat("Gun",pSats) end
    end)
  end
end)
StatsUpg = Tabs.Visuals:Toggle({
    Flag = "Toggle_Auto_Blox_Fruit",
...
spawn(function()
  while wait(Sec) do
    pcall(function()
    if _G.Auto_DevilFruit then Avocado_AddStat("Devil",pSats) end
    end)
  end
end)
StatsUpg = Tabs.Visuals:Toggle({
    Flag = "Toggle_Auto_Defense",
Title = "Auto Defense", 
Desc = "", 
Value = false,
Callback = function(Value)
...
spawn(function()
  while wait(Sec) do
    pcall(function()
    if _G.Auto_Defense then Avocado_AddStat("Defense",pSats) end
    end)
  end
end)

Tabs.Angler:Section({ Title = "Fishing" })

Tabs.Angler:Dropdown({
    Flag = "Dropdown_Select_Fishing_Rod",
...
BaitDropdown = Tabs.Angler:Dropdown({
    Flag = "Dropdown_Select_Bait",
    Title = "Select Bait",
    Desc = "",
...
BuyBaitToggle = Tabs.Angler:Toggle({
    Flag = "Toggle_Auto_Buy_Bait",
    Title = "Auto Buy Bait",
    Desc = "",
    Value = false,
    Callback = function(Value)
...
FishingToggle = Tabs.Angler:Toggle({
    Flag = "Toggle_Auto_Fishing",
    Title = "Auto Fishing",
    Desc = "",
    Value = false,
    Callback = function(Value)
...
FishingQ = Tabs.Angler:Toggle({
    Flag = "Toggle_Auto_Quest_Fishing",
...
QuestToggle = Tabs.Angler:Toggle({
    Flag = "Toggle_Auto_Complete_Quest",
...
SellFishToggle = Tabs.Angler:Toggle({
    Flag = "Toggle_Auto_Sell_Fish",
...
SpamSkillZ = Tabs.Angler:Toggle({
    Flag = "Toggle_Auto_Spam_Skill_Z",
...
TravelDress = 
Tabs.Missions:Section({ Title = "Quest Mode" })
Tabs.Missions:Toggle({ Title = "Single Quest", Flag = "Toggle_Single_Quest", Value = false, Callback = function(Value) _G.SingleQuest = Value if Value then _G.DoubleQuest = false _G.TripleQuest = false end end })
Tabs.Missions:Toggle({ Title = "Double Quest", Flag = "Toggle_Double_Quest", Value = false, Callback = function(Value) _G.DoubleQuest = Value if Value then _G.SingleQuest = false _G.TripleQuest = false end end })
Tabs.Missions:Toggle({ Title = "Triple Quest", Flag = "Toggle_Triple_Quest", Value = false, Callback = function(Value) _G.TripleQuest = Value if Value then _G.SingleQuest = false _G.DoubleQuest = false end end })

Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_Quest_Sea_2",
	      ...
	      elseif workspace.Map.Ice.Door.CanCollide == false and workspace.Map.Ice.Door.Transparency == 1 then
            if Enemies:FindFirstChild("Ice Admiral") then
              for _,xz in pairs(Enemies:GetChildren()) do
                if xz.Name == "Ice Admiral" and CombatCore.Alive(xz) then
              	  repeat task.wait() CombatCore.Kill(xz,_G.TravelDres) until _G.TravelDres == false or xz.Humanoid.Health <= 0
...
Zou = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_Quest_Sea_3",
          ...
          if replicated.Remotes.CommF_:InvokeServer("BartiloQuestProgress","Bartilo") == 3 then
            if replicated.Remotes.CommF_:InvokeServer("GetUnlockables").FlamingoAccess ~= nil then
              ...
              if replicated.Remotes.CommF_:InvokeServer("ZQuestProgress", "Check") == 0 then
                local v = Avocado_FindEnemy("rip_indra")
                if v then
                  repeat wait() CombatCore.Kill(v,_G.AutoZou) until not _G.AutoZou or not v.Parent or v.Humanoid.Health <= 0
                  Check = 2
              ...
              elseif replicated.Remotes["CommF_"]:InvokeServer("ZQuestProgress", "Check") == 1 then
                ...
                local v = Avocado_FindEnemy("Don Swan")
                if v then
                  repeat wait() CombatCore.Kill(v,_G.AutoZou)until not _G.AutoZou or not v.Parent or v.Humanoid.Health<=0                  
                else
            ...
            if replicated.Remotes.CommF_:InvokeServer("BartiloQuestProgress","Bartilo") == 0 then
              if string.find(plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Swan Pirates") and string.find(plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "50") and plr.PlayerGui.Main.Quest.Visible == true then                
                local v = Avocado_FindEnemy("Swan Pirate")
                if v then
                  pcall(function() repeat wait() CombatCore.Kill(v,_G.AutoZou) until not v.Parent or v.Humanoid.Health <= 0 or _G.AutoZou == false or plr.PlayerGui.Main.Quest.Visible == false end)                    
                else
            ...
            elseif replicated.Remotes.CommF_:InvokeServer("BartiloQuestProgress","Bartilo") == 1 then
              local v = Avocado_FindEnemy("Jeremy")
              if v then
                repeat wait() CombatCore.Kill(v,_G.AutoZou) until not v.Parent or v.Humanoid.Health <= 0 or _G.AutoZou == false
              else
...
Tabs.Missions:Section({ Title = "Tushita + Yama" })

Q = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_Tushita_Sword",
    ...
    pcall(function()
      if _G.Auto_Tushita then
        if workspace.Map.Turtle:FindFirstChild("TushitaGate") then
          if not Avocado_HasTool("Holy Torch") then
            _tp(CFrame.new(5148.03613, 162.352493, 910.548218))
            wait(0.7)
          else
            Avocado_EquipTool("Holy Torch")
            task.wait(1)
          ...
          local v = Avocado_FindEnemy("Longma")
          if v then repeat task.wait() CombatCore.Kill(v,_G.Auto_Tushita) until v.Humanoid.Health <= 0 or not _G.Auto_Tushita or not v.Parent
          else 
      ...
      if _G.Auto_Yama then
	    ...
	    elseif replicated.Remotes.CommF_:InvokeServer("EliteHunter", "Progress") > 30 then
	      ...
	      if (workspace.Map.Waterfall.SealedKatana.Handle.Position-plr.Character.HumanoidRootPart.Position).Magnitude >= 20 then
            ...
            local zx = Avocado_FindEnemy("Ghost")
            if zx then
              repeat wait() CombatCore.Kill(zx,_G.Auto_Yama) until zx.Humanoid.Health <= 0 or not zx.Parent or not _G.Auto_Yama               
...
Tabs.Missions:Section({ Title = "Skull Guitars / Misc" })
local CheckSoul = Tabs.Missions:Paragraph({ Title = "Skull Guitar Quests", Desc = "" })
spawn(function()
    while wait(0.2) do
        pcall(function()
            if Quest1 == true then 
            ...
            elseif Avocado_HasWeapon("Skull Guitar") then 
...
task.spawn(function()
  while wait() do
    if _G.Auto_Soul_Guitar then 
      pcall(function() 
        local v = Avocado_FindEnemy("Living Zombie")
        if v then 
        ...
        if World3 then
          ...
          if replicated.Remotes.CommF_:InvokeServer("GuitarPuzzleProgress","Check") == nil then
           ...
           elseif replicated.Remotes.CommF_:InvokeServer("GuitarPuzzleProgress","Check").Swamp == false then
             ...
             Quest4 = false;
             local v = Avocado_FindEnemy("Living Zombie")
             if v then repeat task.wait() CombatCore.Kill(v,_G.Auto_Soul_Guitar) until not _G.Auto_Soul_Guitar or v.Humanoid.Health <= 0 or not v.Parent or workspace.Map["Haunted Castle"].SwampWater.Color ~= Color3.fromRGB(117, 0, 0)
...
Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_Farm_Material_Skull_Guitar",
...
spawn(function()
  while wait(Sec) do
    pcall(function()
      if _G.AutoMatSoul and Avocado_HasWeapon("Skull Guitar") == false then
	    if Avocado_MaterialCount("Bones") >= 500 and Avocado_MaterialCount("Ectoplasm") >= 250 and Avocado_MaterialCount("Dark Fragment") >= 1 then
	      replicated.Remotes.CommF_:InvokeServer("soulGuitarBuy",true)
		else
		  if Avocado_MaterialCount("Ectoplasm") <= 250 then
		    if _G.AutoMatSoul and World2 then
		      local EctoTable = {"Ship Deckhand","Ship Engineer","Ship Steward","Ship Officer","Arctic Warrior"}    
		      local xz = Avocado_FindEnemy(EctoTable)
              if xz then repeat task.wait() CombatCore.Kill(xz, _G.AutoMatSoul)until not _G.AutoMatSoul or not xz.Parent or xz.Humanoid.Health <= 0
		  ...
		  elseif Avocado_MaterialCount("Dark Fragment") < 1 then
		    if _G.AutoMatSoul and World2 then
		      local black = Avocado_FindEnemy("Darkbeard")
		      if black then repeat task.wait()CombatCore.Kill(black, _G.AutoMatSoul)until _G.AutoMatSoul or black.Humanoid.Health <= 0
		      else _tp(CFrame.new(3798.4575195313, 13.826690673828, -3399.806640625))
		      end
		    else replicated.Remotes.CommF_:InvokeServer("TravelDressrosa")
			end
		     if not Avocado_FindEnemy("Darkbeard") then Hop() end
	         elseif Avocado_MaterialCount("Bones") <= 500 then
		       if _G.AutoMatSoul and World3 then
			     local BonesTable = {"Reborn Skeleton","Living Zombie","Demonic Soul","Posessed Mummy"}
			     local zx = Avocado_FindEnemy(BonesTable)			   
	             if zx then repeat task.wait()CombatCore.Kill(zx, _G.AutoMatSoul)until not _G.AutoMatSoul or zx.Humanoid.Health <= 0 or not zx.Parent or zx.Humanoid.Health <= 0
...
Tabs.Missions:Section({ Title = "Cursed Dual Katana" })
local CheckCDK = Tabs.Missions:Paragraph({ Title = "Number Cursed dual katana quests", Desc = "Quest Numbers :" })
spawn(function()  
    while wait(0.2) do 
        ...
        elseif Avocado_HasWeapon("Cursed Dual Katana") then
            CheckCDK:SetDesc("Quest Numbers : CDK done!!")
        end 
    end
end)
Q = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_Get_CDK_Last_Quest",
  ...
  while wait(Sec) do
    pcall(function()
      if _G.CDK then
        ...
        local v = Avocado_FindEnemy("Cursed Skeleton Boss")
        if v then
          repeat wait()
            if plr.Character:FindFirstChild("Yama") or plr.Backpack:FindFirstChild("Yama") then Avocado_EquipTool("Yama")
            elseif plr.Character:FindFirstChild("Tushita") or plr.Backpack:FindFirstChild("Tushita") then Avocado_EquipTool("Tushita")                                    
            ...
            if tonumber(replicated.Remotes.CommF_:InvokeServer("CDKQuest","Progress")["Evil"]) == -3 then
              ...
              repeat task.wait()
                if not workspace.Enemies:FindFirstChild("Forest Pirate") then
                  ...
                  local v = Avocado_FindEnemy("Forest Pirate")
                    ...
                    if (CFramePos.Position - Root.Position).Magnitude <= 1000 and workspace.Enemies:FindFirstChild(NameMonHaze) then
                      for i,v in pairs(workspace.Enemies:GetChildren()) do
                        if v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v:FindFirstChild("Humanoid").Health > 0 and v:FindFirstChild("HazeESP") then
                          repeat wait() CombatCore.Kill(v, _G.CDK_YM) until not _G.CDK_YM or tonumber(replicated.Remotes.CommF_:InvokeServer("CDKQuest","Progress")["Evil"]) == 2 or not v:FindFirstChild("HazeESP") or v.Humanoid.Health <= 0
                        end
                      end
                    else   
                      _tp(CFramePos)                               
                  ...
                  for i,v in pairs(workspace.Enemies:GetChildren()) do
                    if (v:FindFirstChild("HumanoidRootPart").Position - workspace.Map.HellDimension.Spawn.Position).Magnitude <= 300 then
                      if v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v:FindFirstChild("Humanoid").Health > 0 then
                        repeat task.wait() CombatCore.Kill(v,_G.CDK_YM) until not _G.CDK_YM or v.Humanoid.Health <= 0 or not v.Parent or tonumber(replicated.Remotes.CommF_:InvokeServer("CDKQuest","Progress")["Evil"]) == 3
                      end
                    end
                  end
                end
                ...
                if workspace.Enemies:FindFirstChild("Reborn Skeleton") or workspace.Enemies:FindFirstChild("Living Zombie") or workspace.Enemies:FindFirstChild("Domenic Soul") or workspace.Enemies:FindFirstChild("Posessed Mummy") then
                  for i,v in pairs(workspace.Enemies:GetChildren()) do
                    if v.Name == "Reborn Skeleton" or v.Name == "Living Zombie" or v.Name == "Demonic Soul" or v.Name == "Posessed Mummy" then
                      if v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v:FindFirstChild("Humanoid").Health > 0 then
                        repeat task.wait() CombatCore.Kill(v,_G.CDK_YM)until not _G.CDK_YM or v.Humanoid.Health <= 0 or not v.Parent
                      end
                    end
                  end
                else
...
Tabs.Missions:Section({ Title = "True Triple Katana Sword" })
Tabs.Missions:Button({
...
Q = Tabs.Missions:Toggle({
    Flag = "Toggle_Tween_to_Legendary_Sword_Dealer",
...
Tabs.Missions:Section({ Title = "Pole / God Enal's" })
Q = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_Pole_V1",
Title = "Auto Pole V1", 
Desc = "", 
Value = false,
Callback = function(Value)
...
spawn(function()
  while wait(Sec) do
    if _G.AutoPole then
      pcall(function()
        local v = Avocado_FindEnemy("Thunder God")
	    if v then
          repeat task.wait() CombatCore.Kill(v, _G.AutoPole) until not _G.AutoPole or not v.Parent or v.Humanoid.Health <= 0
        else
  ...
  while wait(Sec) do
    pcall(function()
      if _G.AutoPoleV2 then        
	   if not Avocado_HasTool("Pole (1st Form)") then replicated.Remotes.CommF_:InvokeServer("LoadItem","Pole (1st Form)") end
	   if not Avocado_HasTool("Pole (2nd Form)") then replicated.Remotes.CommF_:InvokeServer("LoadItem","Pole (2nd Form)") end      
	   if Avocado_HasTool("Pole (1st Form)") and Avocado_HasTool("Pole (1st Form)").Level.Value <= 179 then _G.Level = true elseif Avocado_HasTool("Pole (1st Form)") and Avocado_HasTool("Pole (1st Form)").Level.Value >= 180 then _G.Level = false end	   
	   if not Avocado_HasTool("Rumble Fruit") then return end
	   if Avocado_HasTool("Rumble Fruit").AwakenedMoves:FindFirstChild("Z") and Avocado_HasTool("Rumble Fruit").AwakenedMoves:FindFirstChild("X") and Avocado_HasTool("Rumble Fruit").AwakenedMoves:FindFirstChild("C") and Avocado_HasTool("Rumble Fruit").AwakenedMoves:FindFirstChild("V") and Avocado_HasTool("Rumble Fruit").AwakenedMoves:FindFirstChild("F") then
	     _G.SelectChip = nil
		 _G.Raiding = false
...
spawn(function()
  while wait(.2) do
    pcall(function()
      if _G.AutoSaw then
        local v = Avocado_FindEnemy("The Saw")
        if v then repeat task.wait() CombatCore.Kill(v, _G.AutoSaw)until _G.AutoSaw == false or v.Humanoid.Health <= 0
      ...
      if _G.AutoSaber and plr.Data.Level.Value >= 200 and not plr.Backpack:FindFirstChild("Saber") and not plr.Character:FindFirstChild("Saber") then
        if workspace.Map.Jungle.Final.Part.Transparency == 0 then
		        ...
		        if replicated.Remotes.CommF_:InvokeServer("ProQuestProgress","SickMan") ~= 0 then
		          ...
		          elseif replicated.Remotes.CommF_:InvokeServer("ProQuestProgress","RichSon") == 0 then
			        if workspace.Enemies:FindFirstChild("Mob Leader") or replicated:FindFirstChild("Mob Leader") then
			         ...
			         for i,v in pairs(workspace.Enemies:GetChildren()) do
				       if v.Name == "Mob Leader" and CombatCore.Alive(v) then
				       repeat task.wait() CombatCore.Kill(v, _G.AutoSaber)until v.Humanoid.Health <= 0 or _G.AutoSaber == false
				       end
				     end
			       end
			     elseif replicated.Remotes.CommF_:InvokeServer("ProQuestProgress","RichSon") == 1 then
			       replicated.Remotes.CommF_:InvokeServer("ProQuestProgress","RichSon")
				   Avocado_EquipTool("Relic")
	     ...
	     if workspace.Enemies:FindFirstChild("Saber Expert") or replicated:FindFirstChild("Saber Expert") then
	       for _,v in pairs(workspace.Enemies:GetChildren()) do
		     if v.Name == "Saber Expert" and CombatCore.Alive(v) then
			   repeat task.wait() CombatCore.Kill(v, _G.AutoSaber) until v.Humanoid.Health <= 0 or _G.AutoSaber == false
...
spawn(function()
  while wait(.2) do
    if _G.AutoColShad then
      pcall(function()
        local v = Avocado_FindEnemy("Cyborg")
	    if v then repeat task.wait()CombatCore.Kill(v, _G.AutoColShad)until _G.AutoColShad == false or not v.Parent or v.Humanoid.Health <= 0
	   ...
	   for _, v in pairs(workspace.Characters:GetChildren()) do
          if v.Name ~= plr.Name then
            if v.Humanoid.Health > 0 and v:FindFirstChild("HumanoidRootPart") and v.Parent and (Root.Position - v.HumanoidRootPart.Position).Magnitude <= 230 then
              repeat task.wait() Avocado_EquipTool(_G.SelectWeapon) _tp(v.HumanoidRootPart.CFrame * CFrame.new(1, 1, 2)) until _G.AutoGetUsoap == false or v.Humanoid.Health <= 0 or not v.Parent or not v:FindFirstChild("HumanoidRootPart") or not v:FindFirstChild("Humanoid")
            end
          end
        end
      end
...
spawn(function()
  while wait(Sec) do
    if _G.Greybeard then
      pcall(function()
        if not Avocado_HasWeapon("Bisento") then
          replicated.Remotes.CommF_:InvokeServer("BuyItem","Bisento")
        elseif Avocado_HasWeapon("Bisento") then
          replicated.Remotes.CommF_:InvokeServer("LoadItem","Bisento")
          local v = Avocado_FindEnemy("Greybeard")
          if v then repeat wait() CombatCore.Kill(v,_G.Greybeard)until _G.Greybeard == false or not v.Parent or v.Humanoid.Health <= 0
...
spawn(function()
  while wait(.1) do
    if _G.WardenBoss then
      pcall(function()
        local v = Avocado_FindEnemy("Chief Warden")
        if v then repeat wait() CombatCore.Kill(v,_G.WardenBoss) until _G.WardenBoss == false or not v.Parent or v.Humanoid.Health <= 0 
...
spawn(function()
  while wait(.1) do
    if _G.MarinesCoat then
      pcall(function()
        local v = Avocado_FindEnemy("Vice Admiral")
        if v then repeat wait() CombatCore.Kill(v, _G.MarinesCoat) until _G.MarinesCoat == false or not v.Parent or v.Humanoid.Health <= 0
...
spawn(function()
  while wait(.1) do
    if _G.SwanCoat then
      pcall(function()
        local v = Avocado_FindEnemy("Swan")
        if v then repeat wait()CombatCore.Kill(v, _G.SwanCoat)until _G.SwanCoat == false or not v.Parent or v.Humanoid.Health <= 0
...
Tabs.Missions:Section({ Title = "Rengoku Sword" })
Q = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_Rengoku_Sword",
  ...
  pcall(function()
    while wait(.1) do
      if _G.IceBossRen then
        local v = Avocado_FindEnemy("Awakened Ice Admiral")
        if v then repeat task.wait()CombatCore.Kill(v,_G.IceBossRen)until _G.IceBossRen == false or not v.Parent or v.Humanoid.Health <= 0
    ...
    pcall(function()
      if _G.KeysRen then
        if plr.Backpack:FindFirstChild(RenMon[3]) or plr.Character:FindFirstChild(RenMon[3]) then
          Avocado_EquipTool(RenMon[3]) wait(.1)
          _tp(CFrame.new(6571.1201171875, 299.23028564453, -6967.841796875))
        else
          local v = Avocado_FindEnemy(RenMon)
          if v then repeat task.wait() CombatCore.Kill(v,_G.KeysRen)until plr.Backpack:FindFirstChild(RenMon[3]) or _G.KeysRen == false or not v.Parent or v.Humanoid.Health <= 0
...
Q = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_Dragon_Trident",
  ...
  while wait(.1) do
    pcall(function()
      if _G.AutoTridentW2 then
        local v = Avocado_FindEnemy("Tide Keeper")
        if v then repeat task.wait() CombatCore.Kill(v,_G.AutoTridentW2)until _G.AutoTridentW2 == false or not v.Parent or v.Humanoid.Health <= 0
  ...
  while wait(.1) do
    pcall(function()
      if _G.LongsWord then
        local v = Avocado_FindEnemy("Diamond")
        if v then repeat task.wait() CombatCore.Kill(v,_G.LongsWord)until _G.LongsWord == false or not v.Parent or v.Humanoid.Health <= 0
...
spawn(function()
  while wait(.1) do
    if _G.BlackSpikey then
      pcall(function()
        local v = Avocado_FindEnemy("Jeremy")
        if v then repeat wait() CombatCore.Kill(v, _G.BlackSpikey)until _G.BlackSpikey == false or not v.Parent or v.Humanoid.Health <= 0
...
Q = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_Dark_Blade_V3",
...
spawn(function()
  while wait(Sec) do
    pcall(function()
      ...
      if not Avocado_HasTool("Dark Blade") then replicated.Remotes.CommF_:InvokeServer("LoadItem","Dark Blade") end
        if Avocado_HasTool("Fist of Darkness") > 1 then
          if not workspace.Enemies:FindFirstChild("Darkbeard") then
            _tp(CFrame.new(3677.08203125, 62.751937866211, -3144.8332519531))
          elseif Avocado_FindEnemy("Darkbeard") and Avocado_HasTool("Fist of Darkness") >= 1 then
...
Q = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_Midnight_Blade",
  ...
  while wait(Sec) do
    pcall(function()
      if _G.AutoEcBoss then
	    if Avocado_MaterialCount("Ectoplasm") >= 99 then
	      replicated.Remotes.CommF_:InvokeServer("Ectoplasm","Buy", 3)	   
	    elseif Avocado_MaterialCount("Ectoplasm") <= 99 then
	      local v = Avocado_FindEnemy("Cursed Captain")
	      if v then repeat wait()CombatCore.Kill(v, _G.AutoEcBoss) until not _G.AutoEcBoss or not v.Parent or v.Humanoid.Health <= 0
	      else
...
spawn(function()
  while wait(.1) do
    if _G.Auto_Def_DarkCoat then
      pcall(function()
        if Avocado_HasTool("Fist of Darkness") and not workspace.Enemies:FindFirstChild("Darkbeard") then          
          _tp(CFrame.new(3677.08203125, 62.751937866211, -3144.8332519531))
        elseif Avocado_FindEnemy("Darkbeard") then
          local v = Avocado_FindEnemy("Darkbeard")          
		  if v then repeat wait()CombatCore.Kill(v,_G.Auto_Def_DarkCoat)until _G.Auto_Def_DarkCoat == false or not v.Parent or v.Humanoid.Helath <= 0 end
        elseif not Avocado_HasTool("Fist of Darkness") and not Avocado_FindEnemy("Darkbeard") then
          repeat wait(.1) _G.AutoFarmChest = true until not _G.Auto_Def_DarkCoat or Avocado_HasTool("Fist of Darkness") or Avocado_FindEnemy("Darkbeard") _G.AutoFarmChest = false
        end
      end)
    end
  end
...
Q = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_Unlocked_DonSwan",
...
spawn(function()
  while wait(.2) do
    if _G.Auto_SwanGG then
      pcall(function()
        local v = Avocado_FindEnemy("Don Swan")
        if v then repeat wait() CombatCore.Kill(v,_G.Auto_SwanGG)until _G.Auto_SwanGG == false or not v.Parent or v.Humanoid.Health <= 0
...
Tabs.Missions:Section({ Title = "Cavender + Twin Hooks + Bigmom" })
Q = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_Bigmom",
Title = "Auto Bigmom", 
Desc = "", 
Value = false,
Callback = function(Value)
...
spawn(function()
  while wait(Sec) do
    if _G.AutoBigmom then
      pcall(function()
        local bx = Avocado_FindEnemy("Cake Queen")
        if bx then repeat task.wait() CombatCore.Kill(bx, _G.AutoBigmom) until not _G.AutoBigmom or not bx.Parent or bx.Humanoid.Health <= 0
...
Q = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_Canvendish_Sword",
  ...
  while wait(Sec) do
    pcall(function()
      if _G.Auto_Cavender then
        local v = Avocado_FindEnemy("Beautiful Pirate")
	    if v then repeat wait() CombatCore.Kill(v,_G.Auto_Cavender)until not _G.Auto_Cavender or v.Humanoid.Health <= 0
...
spawn(function()
  while wait(Sec) do
    pcall(function()
      if _G.TwinHook then
        local v = Avocado_FindEnemy("Captain Elephant")
	    if v then repeat wait()CombatCore.Kill(v,_G.TwinHook)until not _G.TwinHook or v.Humanoid.Health <= 0
	    else
...
spawn(function()
  while wait(Sec) do
    if _G.AutoSerpentBow then
      local v = Avocado_FindEnemy("Hydra Leader")
      if v then	repeat wait() CombatCore.Kill(v,_G.AutoSerpentBow)until not _G.AutoSerpentBow or not v.Parent or v.Humanoid.Health <= 0
...
Q = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_Lei_Accessory",
...
spawn(function()
  while wait(.2) do
    if _G.AutoKilo then
      pcall(function()
        local v = Avocado_FindEnemy("Kilo Admiral")
        if v then repeat task.wait()CombatCore.Kill(v,_G.AutoKilo)until not _G.AutoKilo or not v.Parent or v.Humanoid.Health <= 0
...
Tabs.Missions:Section({ Title = "Buso/Aura Colours" })
Q = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_Teleport_Barista_Cousin",
...
Q = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_Rainbow_Colors",
      ...
      if _G.Auto_Rainbow_Haki then
        if plr.PlayerGui.Main.Quest.Visible == false then
          ...
          elseif plr.PlayerGui.Main.Quest.Visible == true and string.find(plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Stone") then
            local v = Avocado_FindEnemy("Stone")
            if v then
              repeat wait() CombatCore.Kill(v,_G.Auto_Rainbow_Haki) until _G.Auto_Rainbow_Haki == false or v.Humanoid.Health <= 0 or not v.Parent or plr.PlayerGui.Main.Quest.Visible == false
            else
          ...
          elseif plr.PlayerGui.Main.Quest.Visible == true and string.find(plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Hydra Leader") then
            local v = Avocado_FindEnemy("Hydra Leader")
            if v then
              repeat task.wait()CombatCore.Kill(v,_G.Auto_Rainbow_Haki) until _G.Auto_Rainbow_Haki == false or v.Humanoid.Health <= 0 or not v.Parent or plr.PlayerGui.Main.Quest.Visible == false
            else
...
Q = Tabs.Missions:Toggle({
    Flag = "Toggle_Accept_Rainbow_Quest_Faster",
...
Tabs.Missions:Section({ Title = "Instinct / Observation" })
Q = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_Farm_Observation",
...
Q = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_Observation_V2",
  ...
  while wait(Sec) do
    if _G.AutoKenVTWO then
	  ...
	  if plr.PlayerGui.Main.Quest.Visible == true and string.find(plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text,"Defeat 50 Forest Pirates") then
	    local v = Avocado_FindEnemy("Forest Pirate")
        if v then
	      repeat wait() CombatCore.Kill(v,_G.AutoKenVTWO) until not _G.AutoKenVTWO or v.Humanoid.Health <= 0 or plr.PlayerGui.Main.Quest.Visible == false
	    else
	      _tp(Kv2Pos4)
	    end
	  elseif plr.PlayerGui.Main.Quest.Visible == true then 
	  ...
	  if not Avocado_HasTool("Fruit Bowl") then   	    
	    if not Avocado_HasTool("Apple") then
	      replicated.Remotes.CommF_:InvokeServer("requestEntrance",Vector3.new(-12471.169921875, 374.94024658203, -7551.677734375))
	    ...
	    elseif not Avocado_HasTool("Banana") then
	    ...
	    elseif not Avocado_HasTool("Pineapple") then
...
Bartilo = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_Done_Bartilo_Quest",
      ...
      local Qbart = plr.PlayerGui.Main.Quest
        if replicated.Remotes.CommF_:InvokeServer("BartiloQuestProgress","Bartilo") == 0 then
          ...
          if Qbart.Visible == true then
            local v = Avocado_FindEnemy("Swan Pirate")
            if v then
              local x = Avocado_FindEnemy(BartMon)
              if x then
                repeat task.wait()
                  ...
                  else CombatCore.Kill(x,_G.Bartilo_Quest)end
          ...
          elseif replicated.Remotes.CommF_:InvokeServer("BartiloQuestProgress","Bartilo") == 1 then
            _G.Level = false
            local je = Avocado_FindEnemy("Jeremy")
            if je then
              repeat task.wait() CombatCore.Kill(je,_G.Bartilo_Quest) until _G.Bartilo_Quest == false or not je.Parent or je.Humanoid.Health <= 0 or Qbart.Visible == false or not je:FindFirstChild("HumanoidRootPart")                  
            else
...
CitizenQ = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_Done_Citizen_Quest",
      ...
      if _G.CitizenQuest then
        if Lv >= 1800 and replicated.Remotes.CommF_:InvokeServer("CitizenQuestProgress").KilledBandits == false then
          if string.find(plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Forest Pirate") and string.find(plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "50") and plr.PlayerGui.Main.Quest.Visible == true then
            local v = Avocado_FindEnemy("Forest Pirate")
            if v then
              repeat task.wait() CombatCore.Kill(v,_G.CitizenQuest)until _G.CitizenQuest == false or not v.Parent or v.Humanoid.Health <= 0 or plr.PlayerGui.Main.Quest.Visible == false
            else
        ...
        elseif Lv >= 1800 and replicated.Remotes.CommF_:InvokeServer("CitizenQuestProgress").KilledBoss == false then
          ...
          if plr.PlayerGui.Main.Quest.Visible and string.find(plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Captain Elephant") and plr.PlayerGui.Main.Quest.Visible == true then
            if v then
              repeat task.wait() CombatCore.Kill(v,_G.CitizenQuest) until _G.CitizenQuest == false or v.Humanoid.Health <= 0 or not v.Parent or plr.PlayerGui.Main.Quest.Visible == false
            else
...
Q = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_Training_Dummy",
    ...
    if _G.DummyMan then
      pcall(function()
        if plr.PlayerGui.Main.Quest.Visible == false then	
          ...
          local v = Avocado_FindEnemy("Training Dummy")
          if v then
		    repeat wait() CombatCore.Kill(v,_G.DummyMan) until not _G.DummyMan or not v.Parent or v.Humanoid.Health <= 0
	      else
...
Tabs.Missions:Section({ Title = "Fighting Melee Styles" })
SuperHuman = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_Superhuman",
    ...
    pcall(function()
	  ...
	  local M_Frag = plr.Data.Fragments.Value
        if plr:FindFirstChild("WeaponAssetCache") then
          if not Avocado_HasTool("Superhuman") then                    
            if not Avocado_HasTool("Black Leg") then
            if (M_Beli >= 150000) then replicated.Remotes.CommF_:InvokeServer("BuyBlackLeg") end
            elseif Avocado_HasTool("Black Leg") and Avocado_HasTool("Black Leg").Level.Value < 299 then _G.Level = true elseif Avocado_HasTool("Black Leg") and Avocado_HasTool("Black Leg").Level.Value >= 300 then _G.Level = false end                        
            if not Avocado_HasTool("Electro") then
            if (M_Beli >= 500000) then replicated.Remotes.CommF_:InvokeServer("BuyElectro") end
            elseif Avocado_HasTool("Electro") and Avocado_HasTool("Electro").Level.Value < 299 then _G.Level = true elseif Avocado_HasTool("Electro") and Avocado_HasTool("Electro").Level.Value >= 300 then _G.Level = false end                        
            if not Avocado_HasTool("Fishman Karate") then
            if (M_Beli >= 750000) then replicated.Remotes.CommF_:InvokeServer("BuyFishmanKarate") end
            elseif Avocado_HasTool("Fishman Karate") and Avocado_HasTool("Fishman Karate").Level.Value < 299 then _G.Level = true elseif Avocado_HasTool("Fishman Karate") and Avocado_HasTool("Fishman Karate").Level.Value >= 300 then _G.Level = false end                        
            if not Avocado_HasTool("Dragon Claw") then
            if (M_Frag >= 1500) then replicated.Remotes.CommF_:InvokeServer("BlackbeardReward","DragonClaw","2") end
            elseif Avocado_HasTool("Dragon Claw") and Avocado_HasTool("Dragon Claw").Level.Value < 299 then _G.Level = true elseif Avocado_HasTool("Dragon Claw") and Avocado_HasTool("Dragon Claw").Level.Value >= 300 then _G.Level = false end
...
DeathStep = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_DeathStep",
...
spawn(function()
  while wait(Sec) do
    if _G.AutoDeathStep then
      pcall(function()
        if plr:FindFirstChild("WeaponAssetCache") then  
          if not Avocado_HasTool("Death Step") then          
            if not Avocado_HasTool("Black Leg") then replicated.Remotes.CommF_:InvokeServer("BuyBlackLeg") end
            if Avocado_HasTool("Black Leg") and Avocado_HasTool("Black Leg").Level.Value >= 400 then replicated.Remotes.CommF_:InvokeServer("BuyDeathStep") _G.Level = false elseif Avocado_HasTool("Black Leg") and Avocado_HasTool("Black Leg").Level.Value < 399 then _G.Level = true end
            if Avocado_HasTool("Black Leg") or Avocado_HasTool("Black Leg").Level.Value >= 400 then
            if workspace.Map.IceCastle.Hall.LibraryDoor.PhoeyuDoor.Transparency == 0 then            
              if Avocado_HasTool("Library Key") then repeat wait() _tp(CFrame.new(6371.2001953125, 296.63433837890625, -6841.18115234375)) until not _G.AutoDeathStep or (Root.Position == CFrame.new(6371.2001953125, 296.63433837890625, -6841.18115234375).Position)
		        ...
		        elseif not Avocado_HasTool("Library Key") then
		          local v = Avocado_FindEnemy("Awakened Ice Admiral")
		          if v then	repeat wait() CombatCore.Kill(v,_G.AutoDeathStep) until not v.Parent or v.Humanoid.Health <= 0 or _G.AutoDeathStep == false or Avocado_HasTool("Library Key") or Avocado_HasTool("Death Step")
	              else _tp(CFrame.new(5668.9780273438, 28.519989013672, -6483.3520507813))
	              end
		        end		    
              end
...
SharkManV2 = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_Sharkman_Karate",
...
spawn(function() 
  while wait(Sec) do
    if _G.Auto_SharkMan_Karate then
      pcall(function()
        if plr:FindFirstChild("WeaponAssetCache") then  
          if not Avocado_HasTool("Sharkman Karate") then          
            if not Avocado_HasTool("Fishman Karate") then replicated.Remotes.CommF_:InvokeServer("BuyFishmanKarate") end
            if Avocado_HasTool("Fishman Karate") and Avocado_HasTool("Fishman Karate").Level.Value >= 400 then replicated.Remotes.CommF_:InvokeServer("BuySharkmanKarate") _G.Level = false elseif Avocado_HasTool("Fishman Karate") and Avocado_HasTool("Fishman Karate").Level.Value < 399 then _G.Level = true end
            if Avocado_HasTool("Fishman Karate") or Avocado_HasTool("Fishman Karate").Level.Value >= 400 then           
              if Avocado_HasTool("Water Key") then
		      ...
		      elseif not Avocado_HasTool("Water Key") then
		        local v = Avocado_FindEnemy("Tide Keeper")
		        if v then repeat wait() CombatCore.Kill(v,_G.Auto_SharkMan_Karate)until not v.Parent or v.Humanoid.Health <= 0 or _G.Auto_SharkMan_Karate == false or Avocado_HasTool("Water Key") or Avocado_HasTool("Sharkman Karate")		
...
ElectricClaw = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_ElectricClaw",
...
spawn(function()
  while wait(Sec) do
    if _G.Auto_Electric_Claw then
      pcall(function()
        if plr:FindFirstChild("WeaponAssetCache") then 
        if not Avocado_HasTool("Electro") then replicated.Remotes.CommF_:InvokeServer("BuyElectro") end        
          if Avocado_HasTool("Electro") and Avocado_HasTool("Electro").Level.Value >= 400 then
          ...
          elseif Avocado_HasTool("Electro") and Avocado_HasTool("Electro").Level.Value < 400 then
            repeat _G.AutoFarm_Bone = true wait() until not _G.Auto_Electric_Claw or Avocado_HasTool("Electric Claw") _G.AutoFarm_Bone = false
          end
        end       
      end)
    end
...
DragonTalon = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_DragonTalon",
...
spawn(function()
  while wait(Sec) do
    if _G.AutoDragonTalon then
      pcall(function()
        if plr:FindFirstChild("WeaponAssetCache") then 
        if not Avocado_HasTool("Dragon Claw") then replicated.Remotes.CommF_:InvokeServer("BlackbeardReward","DragonClaw","2") end        
          if Avocado_HasTool("Dragon Claw") and Avocado_HasTool("Dragon Claw").Level.Value >= 400 then replicated.Remotes.CommF_:InvokeServer("Bones","Buy",1,1) replicated.Remotes.CommF_:InvokeServer("BuyDragonTalon")
          elseif Avocado_HasTool("Dragon Claw") and Avocado_HasTool("Dragon Claw").Level.Value < 400 then repeat _G.AutoFarm_Bone = true wait() until not _G.AutoDragonTalon or Avocado_HasTool("Dragon Talon") _G.AutoFarm_Bone = false
          end         
        end
      end)
    end
...
Godhuman = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_Godhuman",
Title = "Auto Godhuman", 
Desc = "", 
Value = false,
Callback = function(Value)
    ...
    pcall(function()
      if _G.Auto_God_Human then
        if replicated.Remotes.CommF_:InvokeServer("BuyGodhuman",true) == "Bring me 20 Fish Tails, 20 Magma Ore, 10 Dragon Scales and 10 Mystic Droplets." then
          if Avocado_MaterialCount("Dragon Scale") == false or Avocado_MaterialCount("Dragon Scale") < 10 then
            if World3 then
              Lv = 1575
              _G.Level = true
            else
          ...
          elseif Avocado_MaterialCount("Fish Tail") == false or Avocado_MaterialCount("Fish Tail") < 20 then
            if World3 then
              Lv = 1775
              _G.Level = true
            else
          ...
          elseif Avocado_MaterialCount("Mystic Droplet") == false or Avocado_MaterialCount("Mystic Droplet") < 10 then
            if World2 then
              Lv = 1425
              _G.Level = true
            else
          ...
          elseif Avocado_MaterialCount("Magma Ore") == false or Avocado_MaterialCount("Magma Ore") < 20 then
            if World2 then
              Lv = 1175
              _G.Level = true
            else
...
SanguineArt = Tabs.Missions:Toggle({
    Flag = "Toggle_Auto_SanguineArt",
...
spawn(function()
  while wait(Sec) do
    if _G.Snaguine then
      pcall(function()
        if not Avocado_HasTool("Sanguine Art") then replicated.Remotes.CommF_:InvokeServer("Sanguine Art") end
        if not Avocado_HasTool("Sanguine Art") then
          if Avocado_MaterialCount("Leviathan Heart") >= 1 then print("Completed!!")
          else
          if World3 then _G.DangerSc = "Lv Infinite" _G.SailBoats = true; else _G.SailBoats = false; end end
          if Avocado_MaterialCount("Vampire Fang") <= 19 then
            if World2 then
              local n = Avocado_FindEnemy("Vampire")
              if n then repeat task.wait() CombatCore.Kill(n,_G.Snaguine) until not _G.Snaguine or n.Humanoid.Health <= 0 or not n.Parent
          ...
          if Avocado_MaterialCount("Vampire Fang") >= 20 and Avocado_MaterialCount("Demonic Wisp") <= 19 then
            if World3 then
              local n = Avocado_FindEnemy("Demonic Soul")
		      if n then repeat task.wait() CombatCore.Kill(n,_G.Snaguine) until not _G.Snaguine or n.Humanoid.Health <= 0 or not n.Parent
           ...
           if Avocado_MaterialCount("Vampire Fang") >= 20 and Avocado_MaterialCount("Demonic Wisp") >= 20 and Avocado_MaterialCount("Dark Fragment") <= 1 then
             if World2 then
               local n = Avocado_FindEnemy("Darkbeard")
		       if n then repeat task.wait() CombatCore.Kill(black,_G.Snaguine) until _G.Snaguine or black.Humanoid.Health <= 0 or not black.Parent
		      else _tp(CFrame.new(3798.4575195313, 13.826690673828, -3399.806640625))
		      end
		    else replicated.Remotes.CommF_:InvokeServer("TravelDressrosa")
	        end
...
local FullMOOn = Tabs.Mirage:Paragraph({ Title = "FullMoon Status", Desc = "" })
local Ismirage = Tabs.Mirage:Paragraph({ Title = "Mirage Island Status", Desc = "" })
spawn(function()
    while wait(0.2) do
...
spawn(function()
    while wait(0.2) do
        pcall(function()
            ...
            local moon = Avocado_MoonTexture()
...
Tabs.Mirage:Toggle({
    Flag = "Toggle_Auto_Find_Mirage_Island",
...
Tabs.Mirage:Toggle({
    Flag = "Toggle_Auto_Tween_To_Mirage_Island",
...
Tabs.Mirage:Toggle({
    Flag = "Toggle_Auto_Tween_To_Highest_Point",
...
Tabs.Mirage:Toggle({
    Flag = "Toggle_Change_Transparency_can_see",
...
Tabs.Mirage:Toggle({
    Flag = "Toggle_Auto_Tween_Advanced_Fruit_Dealer",
...
Tabs.Mirage:Toggle({
    Flag = "Toggle_Auto_Collect_Mirage_Chest",
...
Tabs.Mirage:Section({ Title = "Upgrade Races V2 And V3" })
RaceMink = Tabs.Mirage:Toggle({
    Flag = "Toggle_Auto_Upgrade_Mink",
        ...
        if replicated.Remotes.CommF_:InvokeServer("Alchemist","1") ~= 2 then
          ...
          elseif replicated.Remotes.CommF_:InvokeServer("Alchemist","1") == 1 then
            ...
            elseif not plr.Backpack:FindFirstChild("Flower 3") and not plr.Character:FindFirstChild("Flower 3") then
              local v = Avocado_FindEnemy("Swan Pirate")
              if v then repeat wait() CombatCore.Kill(v,_G.Auto_Mink) until Avocado_HasTool("Flower 3") or not v.Parent or v.Humanoid.Health <= 0 or _G.Auto_Mink == false
...
RaceHuman = Tabs.Mirage:Toggle({
    Flag = "Toggle_Auto_Upgrade_Human",
    ...
    pcall(function()
      if _G.Auto_Human then
		...
		elseif replicated.Remotes.CommF_:InvokeServer("Alchemist","1") == 1 then
		  ...
		  elseif not plr.Backpack:FindFirstChild("Flower 3") and not plr.Character:FindFirstChild("Flower 3") then
		    local v = Avocado_FindEnemy("Swan Pirate")
            if v then repeat wait() CombatCore.Kill(v,_G.Auto_Human) until plr.Backpack:FindFirstChild("Flower 3") or not v.Parent or v.Humanoid.Health <= 0 or _G.Auto_Human == false
		    else _tp(CFrame.new(980.0985107421875, 121.331298828125, 1287.2093505859375))end
		  end
		  elseif replicated.Remotes.CommF_:InvokeServer("Alchemist","1") == 2 then
		    replicated.Remotes.CommF_:InvokeServer("Alchemist","3")
		  ...
		  elseif replicated.Remotes.CommF_:InvokeServer("Wenlocktoad","1") == 0 then
		    replicated.Remotes.CommF_:InvokeServer("Wenlocktoad","2")
		  elseif replicated.Remotes.CommF_:InvokeServer("Wenlocktoad","1") == 1 then
		  local v = Avocado_FindEnemy(Human_v3_Mob[1])
          if v then repeat wait()CombatCore.Kill(v,_G.Auto_Human)until v.Humanoid.Health <= 0 or not v.Parent or not _G.Auto_Human			           
	      else _tp(CFrame.new(-2172.7399902344, 103.32216644287, -4015.025390625))
		  end		      
		  local v = Avocado_FindEnemy(Human_v3_Mob[2])
          if v then repeat wait()CombatCore.Kill(v,_G.Auto_Human)until v.Humanoid.Health <= 0 or not v.Parent or not _G.Auto_Human			           
	      else _tp(CFrame.new(2006.9261474609, 448.95666503906, 853.98284912109))
		  end		      
		  local v = Avocado_FindEnemy(Human_v3_Mob[3])
          if v then repeat wait()CombatCore.Kill(v,_G.Auto_Human)until v.Humanoid.Health <= 0 or not v.Parent or not _G.Auto_Human			           
          else _tp(CFrame.new(-1576.7166748047, 198.59265136719, 13.724286079407))
	      end		      		
        end
...
RaceSky = Tabs.Mirage:Toggle({
    Flag = "Toggle_Auto_Upgrade_Angel",
        ...
        if replicated.Remotes.CommF_:InvokeServer("Alchemist","1") ~= -2 then
		  ...
		  elseif replicated.Remotes.CommF_:InvokeServer("Alchemist","1") == 1 then
		    ...
		    elseif not plr.Backpack:FindFirstChild("Flower 3") and not plr.Character:FindFirstChild("Flower 3") then
		      local v = Avocado_FindEnemy("Swan Pirate")
		      if v then
			    repeat wait()CombatCore.Kill(v,_G.Auto_Skypiea)until plr.Backpack:FindFirstChild("Flower 3") or not v.Parent or v.Humanoid.Health <= 0 or _G.Auto_Skypiea == false
		      else
...
RaceFish = Tabs.Mirage:Toggle({
    Flag = "Toggle_Auto_Upgrade_FishMan",
        ...
        if replicated.Remotes.CommF_:InvokeServer("Alchemist","1") ~= -2 then
		  ...
		  elseif replicated.Remotes.CommF_:InvokeServer("Alchemist","1") == 1 then
	        ...
	        elseif not plr.Backpack:FindFirstChild("Flower 3") and not plr.Character:FindFirstChild("Flower 3") then
	          local v = Avocado_FindEnemy("Swan Pirate")
		      if v then
			    repeat wait()CombatCore.Kill(v,_G.Auto_Fish)until plr.Backpack:FindFirstChild("Flower 3") or not v.Parent or v.Humanoid.Health <= 0 or _G.Auto_Fish == false
	          else
		       _tp(CFrame.new(980.0985107421875, 121.331298828125, 1287.2093505859375))
	          end
            end
...
Tabs.Mirage:Section({ Title = "Trials Quest V4" })
local CheckTier = Tabs.Mirage:Paragraph({ Title = "Tiers V4 Status", Desc = "" })
spawn(function()
    pcall(function()
        while wait(0.2) do
...
PullLv = Tabs.Mirage:Toggle({
    Flag = "Toggle_Auto_Pull_Lever",
...
Train = Tabs.Mirage:Toggle({
    Flag = "Toggle_Auto_Train_V4",
Title = "Auto Train V4", 
Desc = "", 
Value = false,
Callback = function(Value)
      ...
      if _G.AcientOne then
	    ...
	    for i=1,#BonesTable do
	      ...
	      elseif plr.Character:FindFirstChild("RaceTransformed").Value == false then
	        local v = Avocado_FindEnemy(BonesTable)
	        if v then repeat wait() CombatCore.Kill(v, _G.AcientOne) until _G.AcientOne == false or v.Humanoid.Health <= 0 or not v.Parent
		    else _tp(CFrame.new(-9495.6806640625, 453.58624267578125, 5977.3486328125)) 
		    end
	      end
        end
...
Doors = Tabs.Mirage:Toggle({
    Flag = "Toggle_Auto_Teleport_to_Race_Doors",
...
Trials = Tabs.Mirage:Toggle({
    Flag = "Toggle_Auto_Complete_Trial_Race",
	    ...
	    if tostring(plr.Data.Race.Value) == "Fishman" then
	      if GetSeaBeastTrial() then            
            repeat task.wait()
              ...
              Avocado_CastSkill("Melee","Z")
	          Avocado_CastSkill("Melee","X")
	          Avocado_CastSkill("Melee","C")
              wait(.1)
              Avocado_CastSkill("Sword","Z")
              Avocado_CastSkill("Sword","X")
              wait(.1)
              Avocado_CastSkill("Blox Fruit","Z")
              Avocado_CastSkill("Blox Fruit","X")
              Avocado_CastSkill("Blox Fruit","C")
    ...
    pcall(function()
      if _G.Complete_Trials then
	    if tostring(plr.Data.Race.Value) == "Human" or tostring(plr.Data.Race.Value) == "Ghoul" then	      
	      ...
	      local v = Avocado_FindEnemy(TrialsTables)
          if v then repeat wait() CombatCore.Kill(v, _G.Complete_Trials)until _G.Complete_Trials == false or not v.Parent or v.Humanoid.Health <= 0 end		
        end
      end
    end)
  end
end)
AutoKill = Tabs.Mirage:Toggle({
    Flag = "Toggle_Auto_Kill_Player_After_Trial",
	    ...
	    for _, v in pairs(workspace.Characters:GetChildren()) do
          if v.Name ~= plr.Name then
            if v.Humanoid.Health > 0 and v:FindFirstChild("HumanoidRootPart") and v.Parent and (Root.Position - v.HumanoidRootPart.Position).Magnitude <= 250 then
              repeat task.wait() Avocado_EquipTool(_G.SelectWeapon) _tp(v.HumanoidRootPart.CFrame * CFrame.new(0,0,15)) sethiddenproperty(plr, "SimulationRadius", math.huge)until _G.Defeating == false or v.Humanoid.Health <= 0 or not v.Parent or not v:FindFirstChild("HumanoidRootPart") or not v:FindFirstChild("Humanoid")
            end
          end
        end
      end
...
Tabs.Volcano:Section({ Title = "Dojo Quest" })
Tabs.Volcano:Button({
...
DojoQ = Tabs.Volcano:Toggle({
    Flag = "Toggle_Auto_Dojo_Trainer",
      ...
      pcall(function()
        ...
        elseif debug == true and (CFrame.new(5865.0234375, 1208.3154296875, 871.15185546875).Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 50 then
          if NameBelt == "White" then
            local v = Avocado_FindEnemy("Skull Slayer")
            if v then repeat task.wait() CombatCore.Kill(v, _G.Dojoo) until not progress or not _G.Dojoo or not CombatCore.Alive(v)
...
BlazeEM = Tabs.Volcano:Toggle({
    Flag = "Toggle_Auto_Dragon_Hunter",
...
DragonMobClear=function(a,b,c)if workspace.Enemies:FindFirstChild(b)then for d,e in pairs(workspace.Enemies:GetChildren())do if e.Name==b and CombatCore.Alive(e)then if a then CombatCore.Kill(e,a)end end end else _tp(c)end end
spawn(function()
  while wait() do 
...
Toggle = Tabs.Volcano:Toggle({
    Flag = "Toggle_Auto_Drago_V1",
Title = "Auto Drago (V1)", 
Desc = "", 
Value = false,
Callback = function(Value)
...
spawn(function()
  while wait(Sec) do
    pcall(function()
      if _G.DragoV1 then     
        if Avocado_MaterialCount("Dragon Egg") <= 0 then
        repeat wait()
          _G.Prehis_Find = true
          _G.Prehis_Skills = true
          _G.Prehis_DE = true
        until not _G.DragoV1 or Avocado_MaterialCount("Dragon Egg") >= 1
          _G.Prehis_Find = false
          _G.Prehis_Skills = false
          _G.Prehis_DE = false
        end
...
fireflower = Tabs.Volcano:Toggle({
    Flag = "Toggle_Auto_Drago_V2",
Title = "Auto Drago (V2)", 
Desc = "", 
Value = false,
Callback = function(Value)
...
spawn(function()
  while wait(Sec) do
    if _G.AutoFireFlowers then
      ...
      local v = Avocado_FindEnemy("Forest Pirate")
      if v then repeat wait() CombatCore.Kill(v,_G.AutoFireFlowers) until not _G.AutoFireFlowers or not v.Parent or v.Humanoid.Health <= 0 or FireFlower
    ...
    pcall(function()
      if _G.TrainDrago then
	    ...
	    for i=1,#DragoM do
	      ...
	      elseif plr.Character:FindFirstChild("RaceTransformed").Value == false then
	        local v = Avocado_FindEnemy(DragoM)
	        if v then repeat wait() CombatCore.Kill(v, _G.TrainDrago) until _G.TrainDrago == false or v.Humanoid.Health <= 0 or not v.Parent                    		
...
bdrago = Tabs.Volcano:Toggle({
    Flag = "Toggle_Swap_Drago_Race",
...
UpTalon = Tabs.Volcano:Toggle({
    Flag = "Toggle_Upgrade_Dragon_Talon_With_Uzoth",
...
Tabs.Volcano:Section({ Title = "Volcanic Crafting" })
...
Tabs.Volcano:Section({ Title = "Prehistoric Island" })
local Check_Volcano = Tabs.Volcano:Paragraph({ Title = "Prehistoric Island Status", Desc = "" })
spawn(function()
    while wait(0.2) do
        if workspace.Map:FindFirstChild("PrehistoricIsland") or workspace._WorldOrigin.Locations:FindFirstChild("Prehistoric Island") then
...
Tabs.Volcano:Toggle({
    Flag = "Toggle_Craft_Volcanic_Magnet",
    Title = "Craft Volcanic Magnet",
    Value = false,
    Callback = function(Value)
...
Tabs.Volcano:Toggle({
    Flag = "Toggle_Auto_Find_Prehistoric_Island",
...
Tabs.Volcano:Toggle({
    Flag = "Toggle_Auto_Start_Prehistoric_Event",
...
Tabs.Volcano:Toggle({
    Flag = "Toggle_Auto_Patch_Prehistoric_Event",
...
spawn(function()
    while wait(Sec) do
        if _G.Prehis_Skills then
            pcall(function()
                local golem = Avocado_FindEnemy("Lava Golem")
                if golem and golem:FindFirstChild("Humanoid") then
                    repeat
                        wait(0.1)
                        CombatCore.Kill(golem, true)
                    ...
                    if glow and glow.Enabled then
                        repeat
                            ...
                            if plr:DistanceFromCharacter(layer.CFrame.Position) <= 150 then
                                ...
                                Avocado_CastSkill("Melee","Z") wait(.4)
                                Avocado_CastSkill("Melee","X") wait(.4)
                                Avocado_CastSkill("Melee","C") wait(.4)
                                Avocado_CastSkill("Blox Fruit","Z") wait(.4)
                                Avocado_CastSkill("Blox Fruit","X") wait(.4)
                                Avocado_CastSkill("Blox Fruit","C")
                            end
...
Kaura = Tabs.Volcano:Toggle({
    Flag = "Toggle_Kill_Aura",
    Title = "Kill Aura",
    Desc = "",
    Value = false,
    Callback = function(Value)
...
Vocan = Tabs.Volcano:Toggle({
    Flag = "Toggle_Auto_Collect_Dino_Bones",
...
Vocan = Tabs.Volcano:Toggle({
    Flag = "Toggle_Auto_Collect_Dragon_Eggs",
...
Toggle = Tabs.Volcano:Toggle({
    Flag = "Toggle_Auto_Reset_When_Complete_Volcano",
...
Tabs.Ocean:Section({ Title = "Sea Event / Setting Sail" })
...
Tabs.Ocean:Toggle({
    Flag = "Toggle_Ship_Speed_Modifier",
...
Tabs.Ocean:Section({ Title = "Crafting Items" })
...
Tabs.Ocean:Section({ Title = "Choose Sea Event" })

Q = Tabs.Ocean:Dropdown({
    Flag = "Dropdown_Select_Boats",
    Title = "Select Boats",
	Values = ListSeaBoat,
	Callback = function(Value)
...
Q = Tabs.Ocean:Dropdown({
    Flag = "Dropdown_Select_Sea_Level",
...
spawn(function()
  while wait() do
    if _G.SailBoats then 
      pcall(function()        
        local myBoat = CheckBoat()
        if not myBoat and not(CheckShark()and _G.Shark or CheckTerrorShark()and _G.TerrorShark or CheckFishCrew()and _G.MobCrew or CheckPiranha()and _G.Piranha)and not(CheckEnemiesBoat()and _G.FishBoat)and not(CheckSeaBeast()and _G.SeaBeast1)and not(_G.PGB and CheckPirateGrandBrigade())and not(_G.HCM and CheckHauntedCrew())and not(_G.Leviathan1 and Avocado_HasLeviathan())then
          local buyBoatCFrame = CFrame.new(-16927.451, 9.086, 433.864)
          TeleportToTarget(buyBoatCFrame)
          if (buyBoatCFrame.Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 10 then replicated.Remotes.CommF_:InvokeServer("BuyBoat", _G.SelectedBoat) end
        elseif myBoat and not(CheckShark()and _G.Shark or CheckTerrorShark()and _G.TerrorShark or CheckFishCrew()and _G.MobCrew or CheckPiranha()and _G.Piranha)and not(CheckEnemiesBoat()and _G.FishBoat)and not(CheckSeaBeast()and _G.SeaBeast1)and not(_G.PGB and CheckPirateGrandBrigade())and not(_G.HCM and CheckHauntedCrew())and not(_G.Leviathan1 and Avocado_HasLeviathan())then
          if plr.Character.Humanoid.Sit == false then
            local boatSeatCFrame = myBoat.VehicleSeat.CFrame * CFrame.new(0, 1, 0)
            _tp(boatSeatCFrame)
          else                         
            ...
            until _G.SailBoats==false or(CheckShark()and _G.Shark or CheckTerrorShark()and _G.TerrorShark or CheckFishCrew()and _G.MobCrew or CheckPiranha()and _G.Piranha)or CheckSeaBeast()and _G.SeaBeast1 or CheckEnemiesBoat()and _G.FishBoat or _G.Leviathan1 and Avocado_HasLeviathan() or _G.HCM and CheckHauntedCrew() or _G.PGB and CheckPirateGrandBrigade() or plr.Character:WaitForChild("Humanoid").Sit==false plr.Character.Humanoid.Sit = false
          end
        end
      end)
    end
...
Tabs.Ocean:Section({ Title = "Entity Sea Event" })

Tabs.Ocean:Toggle({
    Flag = "Toggle_Auto_Shark",
Title = "Auto Shark", 
Desc = "", 
Value = false,
Callback = function(Value)
...
Tabs.Ocean:Toggle({
    Flag = "Toggle_Auto_Fish_Crew_Member",
...
Tabs.Ocean:Toggle({
    Flag = "Toggle_Auto_Haunted_Crew_Member",
...
Tabs.Ocean:Toggle({
    Flag = "Toggle_Auto_Attack_PirateGrandBrigade",
...
Tabs.Ocean:Toggle({
    Flag = "Toggle_Auto_Attack_Fish_Boat",
...
Tabs.Ocean:Toggle({
    Flag = "Toggle_Auto_Attack_Sea_Beast",
...
spawn(function()
  while wait() do
    pcall(function()	
      if _G.Shark then local a={"Shark"}if CheckShark()then for b,c in pairs(workspace.Enemies:GetChildren())do if table.find(a,c.Name)then if CombatCore.Alive(c)then repeat task.wait()CombatCore.Kill(c,_G.Shark)until _G.Shark==false or not c.Parent or c.Humanoid.Health<=0 end end end end end
      if _G.TerrorShark then local a={"Terrorshark"}if CheckTerrorShark()then for b,c in pairs(workspace.Enemies:GetChildren())do if table.find(a,c.Name)then if CombatCore.Alive(c)then repeat task.wait()CombatCore.KillSea(c,_G.TerrorShark)until _G.TerrorShark==false or not c.Parent or c.Humanoid.Health<=0 end end end end end
      if _G.Piranha then local a={"Piranha"}if CheckPiranha()then for b,c in pairs(workspace.Enemies:GetChildren())do if table.find(a,c.Name)then if CombatCore.Alive(c)then repeat task.wait()CombatCore.Kill(c,_G.Piranha)until _G.Piranha==false or not c.Parent or c.Humanoid.Health<=0 end end end end end
      if _G.MobCrew then local a={"Fish Crew Member"}if CheckFishCrew()then for b,c in pairs(workspace.Enemies:GetChildren())do if table.find(a,c.Name)then if CombatCore.Alive(c)then repeat task.wait()CombatCore.Kill(c,_G.MobCrew)until _G.MobCrew==false or not c.Parent or c.Humanoid.Health<=0 end end end end end                 
      if _G.HCM then local a={"Haunted Crew Member"}if CheckHauntedCrew()then for b,c in pairs(workspace.Enemies:GetChildren())do if table.find(a,c.Name)then if CombatCore.Alive(c)then repeat task.wait()CombatCore.Kill(c,_G.HCM)until _G.HCM==false or not c.Parent or c.Humanoid.Health<=0 end end end end end
      if _G.SeaBeast1 then if workspace.SeaBeasts:FindFirstChild("SeaBeast1")then for a,b in pairs(workspace.SeaBeasts:GetChildren())do if b:FindFirstChild("HumanoidRootPart")and b:FindFirstChild("Health")and b.Health.Value>0 then repeat task.wait()spawn(function()_tp(CFrame.new(b.HumanoidRootPart.Position.X,game:GetService("Workspace").Map["WaterBase-Plane"].Position.Y+200,b.HumanoidRootPart.Position.Z))end)if plr:DistanceFromCharacter(b.HumanoidRootPart.CFrame.Position)<=500 then AitSeaSkill_Custom=b.HumanoidRootPart.CFrame;MousePos=AitSeaSkill_Custom.Position;if CheckF()then Avocado_EquipByType("Blox Fruit")Avocado_CastSkill("Blox Fruit","Z")Avocado_CastSkill("Blox Fruit","X")Avocado_CastSkill("Blox Fruit","C")else Avocado_CastSkill("Melee","Z")Avocado_CastSkill("Melee","X")Avocado_CastSkill("Melee","C")wait(.1)Avocado_CastSkill("Sword","Z")Avocado_CastSkill("Sword","X")wait(.1)Avocado_CastSkill("Blox Fruit","Z")Avocado_CastSkill("Blox Fruit","X")Avocado_CastSkill("Blox Fruit","C")wait(.1)Avocado_CastSkill("Gun","Z")Avocado_CastSkill("Gun","X")end end until _G.SeaBeast1==false or not b:FindFirstChild("HumanoidRootPart")or not b.Parent or b.Health.Value<=0 end end end end
      if _G.Leviathan1 then if workspace.SeaBeasts:FindFirstChild("Leviathan")then for a,b in pairs(workspace.SeaBeasts:GetChildren())do if b:FindFirstChild("HumanoidRootPart")and b:FindFirstChild("Leviathan Segment")and b:FindFirstChild("Health")and b.Health.Value>0 then repeat task.wait()spawn(function()_tp(CFrame.new(b.HumanoidRootPart.Position.X,game:GetService("Workspace").Map["WaterBase-Plane"].Position.Y+200,b.HumanoidRootPart.Position.Z))end)if plr:DistanceFromCharacter(b.HumanoidRootPart.CFrame.Position)<=500 then MousePos=b:FindFirstChild("Leviathan Segment").Position;if CheckF()then Avocado_EquipByType("Blox Fruit")Avocado_CastSkill("Blox Fruit","Z")Avocado_CastSkill("Blox Fruit","X")Avocado_CastSkill("Blox Fruit","C")else Avocado_CastSkill("Melee","Z")Avocado_CastSkill("Melee","X")Avocado_CastSkill("Melee","C")wait(.1)Avocado_CastSkill("Sword","Z")Avocado_CastSkill("Sword","X")wait(.1)Avocado_CastSkill("Blox Fruit","Z")Avocado_CastSkill("Blox Fruit","X")Avocado_CastSkill("Blox Fruit","C")wait(.1)Avocado_CastSkill("Gun","Z")Avocado_CastSkill("Gun","X")end end until _G.Leviathan1==false or not b:FindFirstChild("HumanoidRootPart")or not b.Parent or b.Health.Value<=0 end end end end
      if _G.FishBoat then if CheckEnemiesBoat()then for a,b in pairs(workspace.Enemies:GetChildren())do if b:FindFirstChild("Health")and b.Health.Value>0 and b:FindFirstChild("VehicleSeat")then repeat task.wait()spawn(function()if b.Name=="FishBoat"then _tp(b.Engine.CFrame*CFrame.new(0,-50,-25))end end)if plr:DistanceFromCharacter(b.Engine.CFrame.Position)<=150 then AitSeaSkill_Custom=b.Engine.CFrame;MousePos=AitSeaSkill_Custom.Position;if CheckF()then Avocado_EquipByType("Blox Fruit")Avocado_CastSkill("Blox Fruit","Z")Avocado_CastSkill("Blox Fruit","X")Avocado_CastSkill("Blox Fruit","C")else Avocado_CastSkill("Melee","Z")Avocado_CastSkill("Melee","X")Avocado_CastSkill("Melee","C")wait(.1)Avocado_CastSkill("Sword","Z")Avocado_CastSkill("Sword","X")wait(.1)Avocado_CastSkill("Blox Fruit","Z")Avocado_CastSkill("Blox Fruit","X")Avocado_CastSkill("Blox Fruit","C")wait(.1)Avocado_CastSkill("Gun","Z")Avocado_CastSkill("Gun","X")end end until _G.FishBoat==false or not b:FindFirstChild("VehicleSeat")or b.Health.Value<=0 end end end end
      if _G.PGB then if CheckPirateGrandBrigade()then for a,b in pairs(workspace.Enemies:GetChildren())do if b:FindFirstChild("Health")and b.Health.Value>0 and b:FindFirstChild("VehicleSeat")then repeat task.wait()spawn(function()if b.Name=="PirateBrigade"then _tp(b.Engine.CFrame*CFrame.new(0,-30,-10))elseif b.Name=="PirateGrandBrigade"then _tp(b.Engine.CFrame*CFrame.new(0,-50,-50))end end)if plr:DistanceFromCharacter(b.Engine.CFrame.Position)<=150 then AitSeaSkill_Custom=b.Engine.CFrame;MousePos=AitSeaSkill_Custom.Position;if CheckF()then Avocado_EquipByType("Blox Fruit")Avocado_CastSkill("Blox Fruit","Z")Avocado_CastSkill("Blox Fruit","X")Avocado_CastSkill("Blox Fruit","C")else Avocado_CastSkill("Melee","Z")Avocado_CastSkill("Melee","X")Avocado_CastSkill("Melee","C")wait(.1)Avocado_CastSkill("Sword","Z")Avocado_CastSkill("Sword","X")wait(.1)Avocado_CastSkill("Blox Fruit","Z")Avocado_CastSkill("Blox Fruit","X")Avocado_CastSkill("Blox Fruit","C")wait(.1)Avocado_CastSkill("Gun","Z")Avocado_CastSkill("Gun","X")end end until _G.PGB==false or not b:FindFirstChild("VehicleSeat")or b.Health.Value<=0 end end end end
    end)
  end
end)

Tabs.Ocean:Section({ Title = "Kitsune Island / Event" })
local Check_Kitsu = Tabs.Ocean:Paragraph({ Title = "Kitsune Island Status", Desc = "" })
spawn(function()
    while wait(0.2) do
...
Tabs.Ocean:Toggle({
    Flag = "Toggle_Auto_Find_Kitsune_Island",
...
Tabs.Ocean:Toggle({
    Flag = "Toggle_Auto_Teleport_to_Shrine_Actived",
...
Tabs.Ocean:Toggle({
    Flag = "Toggle_Auto_Collect_Azure_Ember",
...
Tabs.Ocean:Toggle({
    Flag = "Toggle_Auto_Trade_Azure_Ember",
...
Tabs.Ocean:Section({ Title = "Frozen Dimension Event" })

local FloD = Tabs.Ocean:Paragraph({ Title = "FrozenDimension Status", Desc = "" })
spawn(function()
    pcall(function()
        while wait(0.2) do
...
local SPYING = Tabs.Ocean:Paragraph({ Title = "Spy Status", Desc = "" })
spawn(function()
    while wait(0.2) do
        pcall(function()
...
Tabs.Ocean:Toggle({
    Flag = "Toggle_Auto_Teleport_Frozen_Dimension",
...
Tabs.Ocean:Toggle({
    Flag = "Toggle_Auto_Drive_To_Hydra_Island",
...
Tabs.Ocean:Toggle({
    Flag = "Toggle_Auto_Attack_Leviathan",
...
Tabs.Visuals:Toggle({
    Flag = "Toggle_Esp_Legendary_Sword",
...
Tabs.Visuals:Toggle({
    Flag = "Toggle_Esp_SeaEvent_Island",
...
Tabs.Visuals:Toggle({
    Flag = "Toggle_Esp_Advanced_Dealer",
...
local stockParagraph = Tabs.Fruits:Paragraph({ Title = "Stock Fruit", Desc = "Loading..." })
...
RandomFF = Tabs.Fruits:Toggle({
    Flag = "Toggle_Auto_Random_Fruit",
...
DropF = Tabs.Fruits:Toggle({
    Flag = "Toggle_Auto_Drop_Fruit",
...
spawn(function()
  while wait(Sec) do
    if _G.DropFruit then
      pcall(function() Avocado_DropFruits() end)
    end
  end
end)
StoredF = Tabs.Fruits:Toggle({
    Flag = "Toggle_Auto_Store_Fruit",
...
spawn(function()
  while wait(Sec) do
    if _G.StoreF then
      pcall(function() Avocado_StoreFruits() end)
    end
  end
end)
TwF = Tabs.Fruits:Toggle({
    Flag = "Toggle_Auto_Tween_to_Fruit",
...
BringF = Tabs.Fruits:Toggle({
    Flag = "Toggle_Auto_Collect_Fruit",
...
spawn(function()
  while wait(Sec) do
    if _G.InstanceF then
      pcall(function() Avocado_CollectFruits(_G.InstanceF) end)
    end
  end
end)

Tabs.Fruits:Dropdown({
    Flag = "Dropdown_Select_Fruit_Shop",
...
Tabs.Fruits:Toggle({
    Flag = "Toggle_Auto_Buy_Fruit_Shop",
...
Q = Tabs.Fruits:Toggle({
    Flag = "Toggle_Auto_Select_Dungeon_Chip",
...
Tabs.Fruits:Toggle({
    Flag = "Toggle_Get_Fruit_In_Inventory_Below_1M",
...
Desc = "",
Callback = function()
  if not Avocado_HasTool("Special Microchip") then replicated.Remotes.CommF_:InvokeServer("RaidsNpc","Select",_G.SelectChip) end
end})
Tabs.Fruits:Button({
Title = "Buy Dungeon Chips [Devil Fruit]", 
Desc = "",
Callback = function()
  if Avocado_HasTool("Special Microchip") then return end
  local FruitPrice = {}
  local FruitStore = {}
  for i,v in next,replicated:WaitForChild("Remotes").CommF_:InvokeServer("GetFruits") do
  ...
  for _,y in pairs(FruitPrice) do    
    for i,v in pairs(DungeonTables) do 
      if not Avocado_HasTool("Special Microchip") then     
        replicated.Remotes.CommF_:InvokeServer("LoadFruit",tostring(y))	      
...
AutoChipBeli = Tabs.Fruits:Toggle({
    Flag = "Toggle_Auto_Buy_Chip_Beli",
...
AutoChipFruit = Tabs.Fruits:Toggle({
    Flag = "Toggle_Auto_Buy_Chip_Devil_Fruit",
...
StartR = Tabs.Fruits:Toggle({
    Flag = "Toggle_Auto_Start_Raid",
...
task.spawn(function()
    while task.wait(Sec) do
        ...
        pcall(function()
            ...
            if not Avocado_HasTool("Special Microchip") then return end
...
Raiding = Tabs.Fruits:Toggle({
    Flag = "Toggle_Auto_Raid_Next_Island",
        ...
        for _,mob in ipairs(workspace.Enemies:GetChildren()) do
            ...
            if eh and ehrp and eh.Health > 0 then
                if (ehrp.Position - islandPos.Position).Magnitude < 450 then
                    ...
                    repeat
                        task.wait()
                        CombatCore.Kill(mob, _G.Raiding)
...
Tabs.Fruits:Toggle({
    Flag = "Toggle_Auto_Teleport_To_Lab",
...
Tabs.Fruits:Section({ Title = "Items Law/Order Sword" })
...
Tabs.Fruits:Toggle({
    Flag = "Toggle_Auto_Buy_Microchip_Law",
...
Tabs.Fruits:Toggle({
    Flag = "Toggle_Auto_Start_Law_Raids",
...
spawn(function()
  while wait(Sec) do
    if _G.AutoLawKak then
      pcall(function()
        local v = Avocado_FindEnemy("Order")
        if v then repeat task.wait() CombatCore.Kill(v, _G.AutoLawKak) until _G.AutoLawKak == false or not v.Parent or v.Humanoid.Health <= 0
...
Tabs.Fruits:Section({ Title = "Raids Dungeons" })
...
Tabs.Player:Section({ Title = "Combat / AimBot" })

local __indexPlayer = Tabs.Player:Paragraph({ Title = "All Players On Server", Desc = "" })
...
local __AimBotTurn = Tabs.Player:Paragraph({ Title = "Aimbot Status", Desc = "" })
...
Tabs.Player:Toggle({
    Flag = "Toggle_Teleport_To_Select_Players",
...
Tabs.Player:Toggle({
    Flag = "Toggle_Spectate_Select_Players",
...
Tabs.Player:Dropdown({
    Flag = "Dropdown_Select_Aim_Method",
...
Tabs.Player:Toggle({
    Flag = "Toggle_Aimbot_Method_Skills",
...
Tabs.Player:Toggle({
    Flag = "Toggle_Aimbot_Camera_Closet_Players",
...
Tabs.Player:Section({ Title = "Quests Players" })
...
Tabs.Player:Toggle({
    Flag = "Toggle_Auto_Get_PlayerQuest",
...
Tabs.Player:Toggle({
    Flag = "Toggle_Auto_Kill_Player_Quest",
                ...
                for _, target in pairs(game:GetService("Workspace").Characters:GetChildren()) do
                    if string.find(game.Players.LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, target.Name) then
                        repeat
                            ...
                            if Avocado_EquipTool then Avocado_EquipTool(_G.SelectWeapon) end
                            Useskill = true
...
Tabs.Player:Section({ Title = "LocalPlayer Settings" })
...
Tabs.Player:Toggle({
    Flag = "Toggle_Instance_Mink_V3_INF",
...
Tabs.Player:Toggle({
    Flag = "Toggle_Instance_Energy_INF",
...
Tabs.Player:Toggle({
    Flag = "Toggle_Instance_Observation_Range_INF",
...
Tabs.Warp:Section({ Title = "Travel - Worlds" })
...
Tabs.Warp:Section({ Title = "Travel - Island" })
Location = {}
for i,v in pairs(workspace["_WorldOrigin"].Locations:GetChildren()) do  
  table.insert(Location ,v.Name)
end
Travelllll = Tabs.Warp:Dropdown({
    Flag = "Dropdown_Select_Travelling",
...
GoIsland = Tabs.Warp:Toggle({
    Flag = "Toggle_Auto_Travel",
Title = "Auto Travel", 
Desc = "", 
Value = false,
Callback = function(Value)
...
Tabs.Warp:Section({ Title = "Travel - Portal" })
if World1 then
  Location_Portal = {
    "Sky",
    "UnderWater"
...
PortalTP = Tabs.Warp:Dropdown({
    Flag = "Dropdown_Select_Portal",
Title = "Select Portal",
...
Tabs.Warp:Section({ Title = "Travel - NPCs" })
for _, v in pairs(replicated.NPCs:GetChildren()) do table.insert(NPCList, v.Name)end
NPCsPos = Tabs.Warp:Dropdown({
    Flag = "Dropdown_Select_NPCs",
Title = "Select NPCs",
Values = NPCList,
Callback = function(Value)
  NPClist = Value
end})
GoNPCs = Tabs.Warp:Toggle({
    Flag = "Toggle_Auto_Tween_to_NPC",
...
Tabs.Market:Section({ Title = "Shop Options" })
Tabs.Market:Button({
Title = "Buy Buso", 
Desc = "",
Callback = function()
...
Tabs.Market:Section({ Title = "Fighting - Style" })
Tabs.Market:Button({
Title = "Buy Black Leg", 
Desc = "",
Callback = function()
...
Tabs.Market:Section({ Title = "Accessory" })
Tabs.Market:Button({
...
Tabs.Market:Section({ Title = "Weapon World1" })
Tabs.Market:Button({
Title = "Buy Cutlass", 
Desc = "",
Callback = function()
...
Tabs.Market:Section({ Title = "Fragments shop" })
Tabs.Market:Button({
...
Tabs.Extra:Section({ Title = "Server - Function" })
Tabs.Extra:Button({
...
Tabs.Extra:AddInput("Input_Input_Job_Id", {
    Title = "Input Job Id",
    Placeholder = "Job ID",
    Callback = function(Value)
        getgenv().Job = Value
...
Tabs.Extra:Section({ Title = "Player Gui / Others" })
...
DisbleChat = Tabs.Extra:Toggle({
    Flag = "Toggle_Disable_Chat_GUI",
...
DisbleLeaderB = Tabs.Extra:Toggle({
    Flag = "Toggle_Disable_Leader_Board_GUI",
...
UnPortal = Tabs.Extra:Toggle({
    Flag = "Toggle_Unlock_All_Portals",
...
Tabs.Extra:Section({ Title = "Graphics / Haki Stats" })

HakiSt = {"State 0","State 1","State 2","State 3","State 4","State 5"}
HakiStat = Tabs.Extra:Dropdown({
    Flag = "Dropdown_Select_Haki_States",
...
rtxM = Tabs.Extra:Toggle({
    Flag = "Toggle_Turn_on_RTX_Mode",
...
Tabs.Extra:Section({ Title = "Configure - God" })
Tabs.Extra:Button({
...
briggt1 = Tabs.Extra:Toggle({
    Flag = "Toggle_Turn_on_Full_Bright",
...
DayN = Tabs.Extra:Dropdown({
    Flag = "Dropdown_Select_Time",
Title = "Select Time",
Desc = "",
Values = {"Day", "Night"},
Value = Day,
...
dayornight = Tabs.Extra:Toggle({
    Flag = "Toggle_Turn_on_Time",
Title = "Turn on Time", 
Desc = "", 
Value = false,
Callback = function(Value)
...
walkWater = Tabs.Extra:Toggle({
    Flag = "Toggle_Turn_on_Walk_on_Water",
...
iceWalk = Tabs.Extra:Toggle({
    Flag = "Toggle_Turn_on_Ice_Walk",
...
function AttackNoCoolDown()
    ...
    local maxHit = 3 -- attack up to 3 mobs at once
    local count = 0
...
function FastAttackModule.GetNearbyTargets(character, folder)
    ...
    for i = 1, #children do
        ...
        if humanoid and rootPart and humanoid.Health > 0 then
            ...
            if distance <= 60 then
                table.insert(nearbyTargets, {target = target, dist = distance})
            end
        end
    end
    table.sort(nearbyTargets, function(a, b) return a.dist < b.dist end)
    local result = {}
    local maxHit = 3 -- attack up to 3 at once
    for i = 1, math.min(maxHit, #nearbyTargets) do
        result[i] = nearbyTargets[i].target
    end
    return result
end
...
function FastAttackModule.GetAllTargets(character)
    ...
    -- hard cap total multi-hit targets at 3
    local maxHit = 3
    if #allTargets > maxHit then
        local trimmed = {}
        for i = 1, maxHit do
            trimmed[i] = allTargets[i]
        end
        return trimmed
    end
    return allTargets
...
function HitRegistrationModule.Execute()
    ...
    -- limit to parts belonging to the 3 closest unique models
    do
        local maxHit = 3
        local order, seen = {}, {}
        for _, pair in ipairs(hitTargets) do
            local model = pair[1]
        ...
        local allowed = {}
        for i = 1, math.min(maxHit, #order) do
            allowed[order[i]] = true
        end
        local trimmed = {}
        for _, pair in ipairs(hitTargets) do
            if allowed[pair[1]] then
                table.insert(trimmed, pair)
            end
        end
        hitTargets = trimmed
    end
...
local function -- force fast attack enabled (do not rely on UI callback)
_G.Seriality = true
_G.ChooseWP = _G.ChooseWP or "Melee"
_B = (_B == nil) and true or _B
StartMainLoops()
    task.spawn(function()
...
-- force fast attack enabled (do not rely on UI callback)
_G.Seriality = true
_G.ChooseWP = _G.ChooseWP or "Melee"
_B = (_B == nil) and true or _B
StartMainLoops()
...
WindUI:Notify({
  Title = "Avocado Hub",
  Content = "Welcome To 🥑Avocado Hub",
