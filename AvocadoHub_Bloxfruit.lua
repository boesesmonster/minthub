...
Avocado_EquipTool = function(text)
  ...
  -- already equipped
  if text and char:FindFirstChild(text) then return end
  local tool = text and (plr.Backpack:FindFirstChild(text) or char:FindFirstChild(text))
  if not tool then
    local tip = _G.ChooseWP or "Melee"
    for _, t in ipairs(plr.Backpack:GetChildren()) do
      if t:IsA("Tool") and t.ToolTip == tip then tool = t; text = t.Name; break end
    end
  end
  if not tool then
    for _, t in ipairs(plr.Backpack:GetChildren()) do
  ...
  if tool and tool.Parent == plr.Backpack then
    ...
    if tool.Name then _G.SelectWeapon = tool.Name end
  end
end
...
task.spawn(function()
  RunSer.Heartbeat:Connect(function()
    pcall(function()      
      ...
      -- ensure a weapon is equipped every heartbeat
      local char = game.Players.LocalPlayer.Character
      if char and not char:FindFirstChildOfClass("Tool") then
        Avocado_EquipTool(_G.SelectWeapon)
      end
      AttackNoCoolDown() 
      local Pretool = char and char:FindFirstChildOfClass("Tool")
