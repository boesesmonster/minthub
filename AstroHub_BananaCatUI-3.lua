...
Converted to Banana Cat Hub / Nousigi Hub UI
Original logic: boesesmonster (thank azeox main functions)
UI replaced: AstroUI/Fluent -> Banana Cat Hub (Nousigi)
]]

if getgenv().__ASTROHUB_LOADED then return end
getgenv().__ASTROHUB_LOADED = true

-- ===================== SETTINGS / SAVE =====================
Settings = {}
local HttpService = game:GetService("HttpService")
local FolderName = "Astro Hub"
local SaveFileName = game.Players.LocalPlayer.Name .. "-BloxFruitAstro.json"

local function SaveSettings(b, t, A)
    if A ~= nil then Settings[b] = Settings[b] or {}; Settings[b][t] = A
    elseif b ~= nil then Settings[b] = t end
    pcall(function()
...
repeat wait() until not game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("LoadingScreen")
...
local function FireButton(b)
    ...
    b.Activated:Connect(function() game:GetService("GuiService").SelectedObject = nil end)
end
...
local mainGui = game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("Main (minimal)")
    or game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("Main")
repeat wait() until mainGui:FindFirstChild("ChooseTeam")

repeat
    task.wait()
    pcall(function()
        local gui = game:GetService("Players").LocalPlayer.PlayerGui
        local m = gui:FindFirstChild("Main (minimal)") or gui:FindFirstChild("Main")
        if not m then return end
        local ct = m:FindFirstChild("ChooseTeam")
        if not ct or not ct.Visible then return end
        if Settings["Select Team"] == "Pirate" then
            FireButton(m.ChooseTeam.Container.Pirates.Frame.TextButton)
        else
            FireButton(m.ChooseTeam.Container.Marines.Frame.TextButton)
        end
        wait(1)
    end)
until (function()
    ...
    local ct = m:FindFirstChild("ChooseTeam")
    return not ct or not ct.Visible
end)()
...
-- ===================== GLOBALS =====================
getgenv().ExploitReq = syn and syn.request or (identifyexecutor and identifyexecutor() == "Fluxus" and request) or http_request or nil

do
  ply = game.Players
  plr = ply.LocalPlayer
  replicated = game:GetService("ReplicatedStorage")
  Lv = 0; pcall(function() Lv = plr.Data.Level.Value end)
  ...
  Energy = 0; pcall(function() Energy = plr.Character.Energy.Value end)
  Root = nil; pcall(function()
...
CommF = replicated.Remotes.CommF_
vu = vim2
...
pcall(function()
    ...
    if net then
        ...
        Remotes.RFCraft = net:FindFirstChild("RF/Craft")
    end
end)
...
Attack.Kill = function(model, Succes)
  if model and Succes then
    ...
    -- Also hit up to 2 nearby mobs of same type (total 3 at once)
    pcall(function()
      local count = 0
      for _, v in ipairs(workspace.Enemies:GetChildren()) do
        if count >= 2 then break end
        if v ~= model and v.Name == model.Name then
          ...
          if hum and root and hum.Health > 0 then
            ...
            if dist <= 50 then
              count = count + 1
              if ToolTip == "Blox Fruit" then
                _tp(root.CFrame * CFrame.new(0,10,0) * CFrame.Angles(0,math.rad(90),0))
              else
                _tp(root.CFrame * CFrame.new(0,30,0) * CFrame.Angles(0,math.rad(180),0))
              end
            end
          end
        end
    ...
    if RandomCFrame then
      wait(.5)_tp(model.HumanoidRootPart.CFrame * CFrame.new(0,30,25))
      wait(.5)_tp(model.HumanoidRootPart.CFrame * CFrame.new(25,30,0))
      wait(.5)_tp(model.HumanoidRootPart.CFrame * CFrame.new(-25,30,0))
      wait(.5)_tp(model.HumanoidRootPart.CFrame * CFrame.new(0,30,25))
      wait(.5)_tp(model.HumanoidRootPart.CFrame * CFrame.new(-25,30,0))
    end
...
BringEnemy = function(Mon)
    ...
    -- Auto-find primary target if not provided
    if not Mon then
    ...
    pcall(function()
        ...
        -- Bring up to 6 mobs of same type to the target position
        local broughtCount = 0
        local MAX_BRING = 6
        for _, v in ipairs(workspace.Enemies:GetChildren()) do
            if broughtCount >= MAX_BRING then break end
            if v ~= Mon then
                ...
                if alive and v.Name == Mon.Name then
                    ...
                    if distance <= 3000 then
                        ...
                        -- Pull mob if we are network owner and it is not yet close
        ...
        -- Lock the primary target as well
...
end) -- end pcall for namecall hook
...
local Quests = {}
local GuideModule = {}
pcall(function() Quests = require(replicated:WaitForChild("Quests", 3)) or {} end)
pcall(function() GuideModule = require(replicated:WaitForChild("GuideModule", 3)) or {} end)
...
-- =============================================
-- UI INITIALIZATION (Banana Cat Hub / Nousigi)
-- =============================================

-- Converted to Banana Cat Hub / Nousigi Hub UI
-- All original logic preserved; UI layer replaced

-- Banana Cat Hub Library
local A = loadstring(game:HttpGet("https://raw.githubusercontent.com/obiiyeuem/vthangsitink/refs/heads/main/zzzz.lua"))()
Main = A.CreateMain({ Title = "Astro Hub", Desc = "" })
getgenv().Options = A.Options
-- Dark Marine Pure Blue palette (applied after GUI loads)
local MARINE      = Color3.fromRGB(0, 51, 102)
local MARINE_PURE = Color3.fromRGB(0, 70, 140)
local MARINE_LIGHT= Color3.fromRGB(0, 90, 160)
local TEXT_BLUE   = Color3.fromRGB(100, 180, 255)

task.spawn(function()
    repeat wait() until game.CoreGui:FindFirstChild("Nousigi Hub GUI")
    task.wait(0.8)
    local gui = game.CoreGui["Nousigi Hub GUI"]

    -- Rebrand title
    pcall(function()
        local label = gui.Main.MainContainer.TopMain.TextLabelMain
        label.Text = '<font color="rgb(0,70,140)">Astro Hub</font>'
        label.RichText = true
    end)
    pcall(function()
        gui.Main.MainContainer.TopMain.Ruafimg.Image = "rbxassetid://96365955041994"
    end)
    ...
    local function paint(inst)
        ...
        if inst:IsA("TextButton") then
            pcall(function()
                inst.BackgroundColor3 = MARINE
                inst.TextColor3 = Color3.fromRGB(220, 235, 255)
            end)
        elseif inst:IsA("ImageButton") then
            pcall(function() inst.BackgroundColor3 = MARINE end)
        elseif inst:IsA("Frame") then
            pcall(function()
                -- Left tab indicator / yellow lines -> marine blue
                if n:find("line") or n:find("indicator") or n:find("accent") or n:find("select") or n:find("bar") or n:find("tab") then
                    inst.BackgroundColor3 = MARINE_PURE
                end
                -- Toggle switch
                if n:find("toggle") or n:find("switch") then
                    inst.BackgroundColor3 = MARINE_PURE
                end
                -- Replace any yellow/gold leftover
                local bc = inst.BackgroundColor3
                if math.abs(bc.R*255 - 255) < 20 and bc.G*255 > 150 and bc.B*255 < 50 then
                    inst.BackgroundColor3 = MARINE_PURE
                end
            end)
        elseif inst:IsA("ImageLabel") then
            pcall(function()
                if n:find("toggle") or n:find("switch") or n:find("line") or n:find("indicator") then
                    inst.ImageColor3 = MARINE_PURE
                end
            end)
        elseif inst:IsA("TextLabel") and (n:find("title") or n:find("page") or n:find("tab")) then
            pcall(function() inst.TextColor3 = TEXT_BLUE end)
        end
    end

    for _, c in ipairs(gui:GetDescendants()) do paint(c) end
    gui.DescendantAdded:Connect(function(c) task.defer(paint, c) end)

    -- Periodic re-apply (lib sometimes resets colors)
    task.spawn(function()
        while gui and gui.Parent do
            for _, c in ipairs(gui:GetDescendants()) do
                if c:IsA("TextButton") then
                    pcall(function() c.BackgroundColor3 = MARINE; c.TextColor3 = Color3.fromRGB(220,235,255) end)
                end
                local n2 = c.Name:lower()
                if c:IsA("Frame") and (n2:find("line") or n2:find("indicator") or n2:find("tabline") or n2:find("selectbar") or n2:find("accent")) then
                    pcall(function() c.BackgroundColor3 = MARINE_PURE end)
                end
                if (n2:find("toggle") or n2:find("switch")) and (c:IsA("Frame") or c:IsA("ImageLabel")) then
                    pcall(function()
                        c.BackgroundColor3 = MARINE_PURE
                        if c:IsA("ImageLabel") then c.ImageColor3 = MARINE_PURE end
                    end)
                end
            end
            task.wait(2)
...
-- Label proxy: allows :SetDesc() calls to update label text
local function MakeLabel(section, title, content)
    local labelRef = section.CreateLabel({Title = title .. (content and (" " .. tostring(content)) or "")})
    local proxy = {_label = labelRef, _title = title}
    function proxy:SetDesc(text)
        -- Update title text of label via metatable trick
        pcall(function()
            if self._label then
                self._label.Title = tostring(text or "")
            end
        end)
    end
    return proxy
...
-- Pages
PageInfo = Main.CreatePage({Page_Name = "Info & Status", Page_Title = "Info & Status"})
PageMain = Main.CreatePage({Page_Name = "Farming", Page_Title = "Farming"})
PageSettings = Main.CreatePage({Page_Name = "Settings", Page_Title = "Settings"})
PageFish = Main.CreatePage({Page_Name = "Fishing", Page_Title = "Fishing"})
PageQuests = Main.CreatePage({Page_Name = "Quest & Item", Page_Title = "Quest & Item"})
PageSeaEvent = Main.CreatePage({Page_Name = "Sea Event", Page_Title = "Sea Event"})
PageRace = Main.CreatePage({Page_Name = "Mirage & Race", Page_Title = "Mirage & Race"})
PagePrehistoric = Main.CreatePage({Page_Name = "Volcano Event", Page_Title = "Volcano Event"})
PageEsp = Main.CreatePage({Page_Name = "Stats & Esp", Page_Title = "Stats & Esp"})
PageRaids = Main.CreatePage({Page_Name = "Fruit & Raid", Page_Title = "Fruit & Raid"})
PageCombat = Main.CreatePage({Page_Name = "Local Player", Page_Title = "Local Player"})
PageTravel = Main.CreatePage({Page_Name = "Teleport", Page_Title = "Teleport"})
PageShop = Main.CreatePage({Page_Name = "Shopping", Page_Title = "Shopping"})
PageMisc = Main.CreatePage({Page_Name = "Misc", Page_Title = "Misc"})
PageBoss = Main.CreatePage({Page_Name = "Hop Boss", Page_Title = "Hop Boss"})

-- Options alias for dropdowns
getgenv().Options = A.Options or {}

-- Toggle value tracking
local _Toggles = {}
...
_Sec_Info_1 = PageInfo.CreateSection("Information")

_Sec_Info_2 = PageInfo.CreateSection("Status Server")

local TimeZone = MakeLabel(_Sec_Info_2, "Time Zone", "")
...
local GameTime = MakeLabel(_Sec_Info_2, "Game Time", "")
...
local MirageCheck = MakeLabel(_Sec_Info_2, "Mirage Island", "Status: ")
...
local KitsuneCheck = MakeLabel(_Sec_Info_2, "Kitsune Island", "Status: ")
...
local PrehistoricCheck = MakeLabel(_Sec_Info_2, "Prehistoric Island", "Status: ")
...
local FrozenCheck = MakeLabel(_Sec_Info_2, "Frozen Dimension", "Status: ")
...
local CakePrinceStatus = MakeLabel(_Sec_Info_2, "Cake Prince", "")
...
local RipIndraCheck = MakeLabel(_Sec_Info_2, "Rip Indra", "Status: ")
...
local DoughKingCheck = MakeLabel(_Sec_Info_2, "Dough King", "Status: ")
...
local FullMoonCheck = MakeLabel(_Sec_Info_2, "Full Moon", "")
...
local LegendarySwordCheck = MakeLabel(_Sec_Info_2, "Legendary Sword", "Status: ")
...
local BoneCount = MakeLabel(_Sec_Info_2, "Bone", "")
...
WeaponDropdown = _Dropdowns["Dropdown_Select_Weapon"] = {idx=1, vals={}}; _Sec_Main_15.CreateButton({
    Title = "Select Weapon"
    Default = "Melee"}, function(Value)
    _G.ChooseWP = Value
end})
...
_Dropdowns["Dropdown_UI_Scale"] = {idx=1, vals={}}; _Sec_Main_15.CreateButton({
    Title = "UI Scale"
    Default = "Normal"}, function(Value)
        local scales = {Small = 0.8, Normal = 1.0, Big = 1.2}
        SetUIScale(scales[Value])
    end
})

_Sec_Main_3 = PageMain.CreateSection("Farming")

FarmLevel = _Sec_Main_15.CreateToggle({
    Title = "Auto Farm Level",
    Default = false}, function(Value)
        _G.Level = Value
        if not Value then
...
ClosetMons = _Sec_Main_15.CreateToggle({
...
FactoryRaids = _Sec_Main_15.CreateToggle({
...
CastleRaids = _Sec_Main_15.CreateToggle({
...
Ecto = _Sec_Main_15.CreateToggle({
...
_Sec_Main_4 = PageMain.CreateSection("Chest")

ChestTW = _Sec_Main_15.CreateToggle({
...
ChestBP = _Sec_Main_15.CreateToggle({
...
StopI = _Sec_Main_15.CreateToggle({
Title = "Stop Items", 
Default = true}, function(Value)
...
_Sec_Main_5 = PageMain.CreateSection("Collect Berry")

Berry = _Sec_Main_15.CreateToggle({
...
BerryH = _Sec_Main_15.CreateToggle({
...
_Sec_Main_6 = PageMain.CreateSection("Farm Mob")
if World1 then
    _Dropdowns["Dropdown_Select_Mob"] = {idx=1, vals={}}; _Sec_Main_15.CreateButton({
        Title = "Select Mob",
        Default = Bandit}, function(Value)
            getgenv().SelectMob = Value
        end
    })
end
if World2 then
    _Dropdowns["Dropdown_Select_Mob_2"] = {idx=1, vals={}}; _Sec_Main_15.CreateButton({
        Title = "Select Mob",
        Default = Raider}, function(Value)
            getgenv().SelectMob = Value
        end
    })
end
if World3 then
    _Dropdowns["Dropdown_Select_Mob_3"] = {idx=1, vals={}}; _Sec_Main_15.CreateButton({
        Title = "Select Mob"}, function(Value)
            getgenv().SelectMob = Value
        end
    })
end
_Sec_Main_15.CreateToggle({
    Title = "Auto Kill Mob",
    Default = false}, function(Value)
...
_Sec_Main_7 = PageMain.CreateSection("Farm All Island")
...
if World1 then
    _Dropdowns["Dropdown_Select_Island"] = {idx=1, vals={}}; _Sec_Main_15.CreateButton({
        Title = "Select Island"}, function(Value)
...
if World2 then
    _Dropdowns["Dropdown_Select_Island_2"] = {idx=1, vals={}}; _Sec_Main_15.CreateButton({
        Title = "Select Island"}, function(Value)
            _G.SelectIsland = Value
        end
    })
...
if World3 then
    _Dropdowns["Dropdown_Select_Island_3"] = {idx=1, vals={}}; _Sec_Main_15.CreateButton({
        Title = "Select Island"}, function(Value)
            _G.SelectIsland = Value
        end
    })
...
_Sec_Main_8 = PageMain.CreateSection("Farm Elite Hunter")

local Process = MakeLabel(_Sec_Main_15, "Elites Process", "")
spawn(function()
    while wait(Sec) do
        pcall(function()    
...
local EliteHunter = MakeLabel(_Sec_Main_15, "Elite Spawn", "Status: ")
spawn(function()
...
EliteQ = _Sec_Main_15.CreateToggle({
...
EliteH = _Sec_Main_15.CreateToggle({
...
_Sec_Main_9 = PageMain.CreateSection("Farm Rip Indra")
...
_Sec_Main_10 = PageMain.CreateSection("Farming Cake")
local MobKilled = MakeLabel(_Sec_Main_15, "Cake Princes", "")
spawn(function()
    while wait(0.2) do
        pcall(function()
...
Cake = _Sec_Main_15.CreateToggle({
...
CakeQ = _Sec_Main_15.CreateToggle({
Title = "Accept Quests", 
Default = false}, function(Value)
...
CakeSM = _Sec_Main_15.CreateToggle({
...
_Sec_Main_11 = PageMain.CreateSection("Farming Bone")

local CheckingBone = MakeLabel(_Sec_Main_15, "Bones", "")
spawn(function()
    while wait(0.2) do
        pcall(function()
...
BoneQ = _Sec_Main_15.CreateToggle({
Title = "Accept Quests", 
Default = false}, function(Value)
...
RanBone = _Sec_Main_15.CreateToggle({
...
Lucky = _Sec_Main_15.CreateToggle({
...
Pray = _Sec_Main_15.CreateToggle({
...
_Sec_Main_12 = PageMain.CreateSection("Tyrant of the Skies")

local TyrantStatus = MakeLabel(_Sec_Main_15, "Boss Spawn", "")
spawn(function()
    pcall(function()
        while wait(1) do
...
local EyeStatus = MakeLabel(_Sec_Main_15, "Check Status Eyes", "")
...
FarmTyrant = _Sec_Main_15.CreateToggle({
...
FarmPhaBinh = _Sec_Main_15.CreateToggle({
...
_Sec_Main_13 = PageMain.CreateSection("Farm Material")

Test = _Dropdowns["Dropdown_Choose_Material"] = {idx=1, vals={}}; _Sec_Main_15.CreateButton({
Title = "Choose Material",
		Values = MaterialList}, function(Value)
			getgenv().SelectMaterial = Value
		end
		})
Toggle = _Sec_Main_15.CreateToggle({
...
_Sec_Main_14 = PageMain.CreateSection("Farm Boss")

		BossDropdown = _Dropdowns["Dropdown_Select_Boss"] = {idx=1, vals={}}; _Sec_Main_15.CreateButton({
		Title = "Select Boss",
		Values = BossList}, function(value)
			_G.FindBoss = value
		end
		})

FarmBoss = _Sec_Main_15.CreateToggle({
...
BossQ = _Sec_Main_15.CreateToggle({
    Title = "Accept Quests",
    Default = true}, function(Value)
...
FarmAllBoss = _Sec_Main_15.CreateToggle({
...
_Sec_Main_15 = PageMain.CreateSection("Farming Mastery")
local posMastery = {"Cake","Bone"}
local Mastery_Config = _Dropdowns["Dropdown_Choose_Island"] = {idx=1, vals={}}; _Sec_Main_15.CreateButton({
Title = "Choose Island",
		Values = posMastery,
		Default = Bone}, function(Value)
  SelectIsland = Value
end})
local MasteryFruits = _Sec_Main_15.CreateToggle({
...
local MasteryGun = _Sec_Main_15.CreateToggle({
...
local MasterySword = _Sec_Main_15.CreateToggle({
...
_Sec_Settings_16 = PageSettings.CreateSection("Settings / Configure")

Initialize = _Sec_Settings_16.CreateToggle({
Title = "Fast Attack", 
Default = true}, function(Value)
  _G.Seriality = Value
end})
Bringmob = _Sec_Settings_16.CreateToggle({
Title = "Bring Mobs", 
Default = true}, function(Value)
  _B = Value
end})
_Sec_Settings_16.CreateToggle({
...
_Sec_Settings_16.CreateButton({
    Title = "Hop Delay (Minutes)",
    Default = 30}, function(Value)
...
BusuAura = _Sec_Settings_16.CreateToggle({
...
RaceV3Aura = _Sec_Settings_16.CreateToggle({
Title = "Auto Turn on Race V3", 
Default = false,
Flag = "AutoTurnonRaceV3"}, function(Value)
...
RaceV4Aura = _Sec_Settings_16.CreateToggle({
...
RandomAround = _Sec_Settings_16.CreateToggle({
Title = "Auto Turn on Spin  xyz", 
Default = false}, function(Value)
  RandomCFrame = Value
end})
SafeModes = _Sec_Settings_16.CreateToggle({
Title = "Safe Mode", 
Default = false}, function(Value)
  _G.Safemode = Value
end})
...
DisableHitVFX = _Sec_Settings_16.CreateToggle({
...
RmvVFX = _Sec_Settings_16.CreateToggle({
...
DisblesNotify = _Sec_Settings_16.CreateToggle({
...
_Sec_Esp_17 = PageEsp.CreateSection("Stats Upgrade")

StatusSelect = _Sec_Esp_43.CreateButton({
Title = "Stats Value",
Default = 10}, function(Value)
  pSats = Value
end})

StatsUpg = _Sec_Esp_43.CreateToggle({
Title = "Auto Melee", 
Default = false}, function(Value)
  _G.Auto_Melee = Value
end})
...
_Sec_Fish_18 = PageFish.CreateSection("Fishing")

_Dropdowns["Dropdown_Select_Fishing_Rod"] = {idx=1, vals={}}; _Sec_Fish_18.CreateButton({
    Title = "Select Fishing Rod"
    Default = "Fishing Rod"}, function(Value)
        _G.SelectedRod = Value
    end
})

BaitDropdown = _Dropdowns["Dropdown_Select_Bait"] = {idx=1, vals={}}; _Sec_Fish_18.CreateButton({
    Title = "Select Bait"
    Default = "Basic Bait"}, function(Value)
...
BuyBaitToggle = _Sec_Fish_18.CreateToggle({
    Title = "Auto Buy Bait",
    Default = false}, function(Value)
...
FishingToggle = _Sec_Fish_18.CreateToggle({
    Title = "Auto Fishing",
    Default = false}, function(Value)
...
local Config = {}
local GetWaterHeight = function() return 0 end
pcall(function() Config = require(FishReplicated.FishingClient.Config) end)
pcall(function() GetWaterHeight = require(ReplicatedStorage.Util.GetWaterHeightAtLocation) end)
...
FishingQ = _Sec_Fish_18.CreateToggle({
...
QuestToggle = _Sec_Fish_18.CreateToggle({
...
SellFishToggle = _Sec_Fish_18.CreateToggle({
...
SpamSkillZ = _Sec_Fish_18.CreateToggle({
...
TravelDress = _Sec_Quests_28.CreateToggle({
...
Zou = _Sec_Quests_28.CreateToggle({
...
_Sec_Quests_19 = PageQuests.CreateSection("Tushita + Yama")

Q = _Sec_Quests_28.CreateToggle({
...
_Sec_Quests_20 = PageQuests.CreateSection("Skull Guitars / Misc")
local CheckSoul = MakeLabel(_Sec_Quests_28, "Skull Guitar Quests", "")
spawn(function()
    while wait(0.2) do
        pcall(function()
            if Quest1 == true then 
...
_Sec_Quests_21 = PageQuests.CreateSection("Cursed Dual Katana")
local CheckCDK = MakeLabel(_Sec_Quests_28, "Number Cursed dual katana quests", "Quest Numbers :")
spawn(function()  
    while wait(0.2) do 
...
_Sec_Quests_22 = PageQuests.CreateSection("True Triple Katana Sword")
_Sec_Quests_28.CreateButton({
Title = "Buy Legendary Sword"}, function()
...
_Sec_Quests_28.CreateButton({
Title = "Buy True Triple Katana Sword"}, function()
...
_Sec_Quests_23 = PageQuests.CreateSection("Pole / God Enal's")
Q = _Sec_Quests_28.CreateToggle({
Title = "Auto Pole V1", 
Default = false}, function(Value)
  _G.AutoPole = Value
...
_Sec_Quests_24 = PageQuests.CreateSection("Rengoku Sword")
Q = _Sec_Quests_28.CreateToggle({
...
_Sec_Quests_25 = PageQuests.CreateSection("Cavender + Twin Hooks + Bigmom")
Q = _Sec_Quests_28.CreateToggle({
Title = "Auto Bigmom", 
Default = false}, function(Value)
  _G.AutoBigmom = Value
...
_Sec_Quests_26 = PageQuests.CreateSection("Buso/Aura Colours")
Q = _Sec_Quests_28.CreateToggle({
...
_Sec_Quests_28.CreateButton({
Title = "Buy Buso Colors"}, function()
...
_Sec_Quests_27 = PageQuests.CreateSection("Instinct / Observation")
Q = _Sec_Quests_28.CreateToggle({
...
Bartilo = _Sec_Quests_28.CreateToggle({
...
CitizenQ = _Sec_Quests_28.CreateToggle({
...
_Sec_Quests_28 = PageQuests.CreateSection("Fighting Melee Styles")
SuperHuman = _Sec_Quests_28.CreateToggle({
...
DeathStep = _Sec_Quests_28.CreateToggle({
...
SharkManV2 = _Sec_Quests_28.CreateToggle({
...
ElectricClaw = _Sec_Quests_28.CreateToggle({
...
DragonTalon = _Sec_Quests_28.CreateToggle({
...
Godhuman = _Sec_Quests_28.CreateToggle({
Title = "Auto Godhuman", 
Default = false}, function(Value)
...
SanguineArt = _Sec_Quests_28.CreateToggle({
...
_Sec_Race_29 = PageRace.CreateSection("Mystic Island / Full Moon")
local FullMOOn = MakeLabel(_Sec_Race_31, "FullMoon Status", "")
local Ismirage = MakeLabel(_Sec_Race_31, "Mirage Island Status", "")
spawn(function()
    while wait(0.2) do
...
_Sec_Race_31.CreateButton({
Title = "Talk With Stone"}, function()
...
_Sec_Race_30 = PageRace.CreateSection("Upgrade Races V2 And V3")
RaceMink = _Sec_Race_31.CreateToggle({
...
RaceHuman = _Sec_Race_31.CreateToggle({
...
RaceSky = _Sec_Race_31.CreateToggle({
...
RaceFish = _Sec_Race_31.CreateToggle({
...
_Sec_Race_31 = PageRace.CreateSection("Trials Quest V4")
local CheckTier = MakeLabel(_Sec_Race_31, "Tiers V4 Status", "")
spawn(function()
    pcall(function()
        while wait(0.2) do
...
PullLv = _Sec_Race_31.CreateToggle({
...
Train = _Sec_Race_31.CreateToggle({
Title = "Auto Train V4", 
Default = false}, function(Value)
  _G.AcientOne = Value
end})
...
_Sec_Race_31.CreateButton({
    Title = "Teleport to Temple of Time"}, function()
...
_Sec_Race_31.CreateButton({
Title = "Teleport to Ancient One"}, function()
        local plr = game:GetService("Players").LocalPlayer
        local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
...
_Sec_Race_31.CreateButton({
Title = "Teleport to Ancient Clock"}, function()
        local plr = game:GetService("Players").LocalPlayer
        local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
...
Doors = _Sec_Race_31.CreateToggle({
...
Trials = _Sec_Race_31.CreateToggle({
...
AutoKill = _Sec_Race_31.CreateToggle({
...
_Sec_Prehistoric_32 = PagePrehistoric.CreateSection("Dojo Quest")
_Sec_Prehistoric_35.CreateButton({
    Title = "Teleport To Dragon Dojo"}, function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(5661.5322265625, 1013.0907592773438, - 334.9649963378906))
        topos(CFrame.new(5814.42724609375, 1208.3267822265625, 884.5785522460938))
    end
})
DojoQ = _Sec_Prehistoric_35.CreateToggle({
...
BlazeEM = _Sec_Prehistoric_35.CreateToggle({
...
_Sec_Prehistoric_33 = PagePrehistoric.CreateSection("Drago Trial")
...
Toggle = _Sec_Prehistoric_35.CreateToggle({
...
fireflower = _Sec_Prehistoric_35.CreateToggle({
Title = "Auto Drago (V2)", 
Default = false}, function(Value)
...
dragoTpVolcano = _Sec_Prehistoric_35.CreateToggle({
...
bdrago = _Sec_Prehistoric_35.CreateToggle({
...
UpTalon = _Sec_Prehistoric_35.CreateToggle({
...
_Sec_Prehistoric_34 = PagePrehistoric.CreateSection("Volcanic Crafting")

_Sec_Prehistoric_35.CreateButton({
Title = "Craft Dragonheart"}, function()
        local args = {
            [1] = "CraftItem",
            [2] = "Craft",
            [3] = "Dragonheart"
...
_Sec_Prehistoric_35.CreateButton({
Title = "Craft Dragonstorm"}, function()
        local args = {
            [1] = "CraftItem",
            [2] = "Craft",
            [3] = "Dragonstorm"
...
_Sec_Prehistoric_35.CreateButton({
    Title = "Craft Dino Hood"}, function()
        local args = {
            [1] = "CraftItem",
            [2] = "Craft",
            [3] = "DinoHood"
...
_Sec_Prehistoric_35.CreateButton({
    Title = "Craft T-Rex Skull"}, function()
        local args = {
            [1] = "CraftItem",
            [2] = "Craft",
            [3] = "TRexSkull"
...
_Sec_Prehistoric_35 = PagePrehistoric.CreateSection("Prehistoric Island")
local Check_Volcano = MakeLabel(_Sec_Prehistoric_35, "Prehistoric Island Status", "")
spawn(function()
    while wait(0.2) do
        if workspace.Map:FindFirstChild("PrehistoricIsland") or workspace._WorldOrigin.Locations:FindFirstChild("Prehistoric Island") then
...
_Sec_Prehistoric_35.CreateButton({
    Title = "Craft Volcanic Magnet"}, function()
...
_Sec_Prehistoric_35.CreateToggle({
...
Kaura = _Sec_Prehistoric_35.CreateToggle({
    Title = "Kill Aura",
    Default = false}, function(Value)
...
Vocan = _Sec_Prehistoric_35.CreateToggle({
...
_Sec_SeaEvent_36 = PageSeaEvent.CreateSection("Sea Event / Setting Sail")
...
_Sec_SeaEvent_42.CreateButton({
    Title = "Remove Lighting Effect"}, function()
        game:GetService("Lighting").BaseAtmosphere:Destroy()
    end
})

_Sec_SeaEvent_42.CreateToggle({
...
_Sec_SeaEvent_37 = PageSeaEvent.CreateSection("Crafting Items")

_Sec_SeaEvent_42.CreateButton({
Title = "Craft SharkTooth"}, function()
        local args = {
            [1] = "CraftItem",
            [2] = "Craft",
            [3] = "SharkTooth"
...
_Sec_SeaEvent_42.CreateButton({
Title = "Craft TerrorJaw"}, function()
        local args = {
            [1] = "CraftItem",
            [2] = "Craft",
            [3] = "TerrorJaw"
...
_Sec_SeaEvent_42.CreateButton({
Title = "Craft SharkAnchor"}, function()
        local args = {
            [1] = "CraftItem",
            [2] = "Craft",
            [3] = "SharkAnchor"
...
_Sec_SeaEvent_42.CreateButton({
Title = "Craft LeviathanCrown"}, function()
        local args = {
            [1] = "CraftItem",
            [2] = "Craft",
            [3] = "LeviathanCrown"
...
_Sec_SeaEvent_42.CreateButton({
Title = "Craft LeviathanShield"}, function()
        local args = {
            [1] = "CraftItem",
            [2] = "Craft",
            [3] = "LeviathanShield"
...
_Sec_SeaEvent_42.CreateButton({
Title = "Craft LeviathanBoat"}, function()
        local args = {
            [1] = "CraftItem",
            [2] = "Craft",
            [3] = "LeviathanBoat"
...
_Sec_SeaEvent_42.CreateButton({
Title = "Craft LegendaryScroll"}, function()
        local args = {
            [1] = "CraftItem",
            [2] = "Craft",
            [3] = "LegendaryScroll"
...
_Sec_SeaEvent_42.CreateButton({
Title = "Craft MythicalScroll"}, function()
        local args = {
            [1] = "CraftItem",
            [2] = "Craft",
            [3] = "MythicalScroll"
...
_Sec_SeaEvent_38 = PageSeaEvent.CreateSection("Choose Sea Event")

Q = _Dropdowns["Dropdown_Select_Boats"] = {idx=1, vals={}}; _Sec_SeaEvent_42.CreateButton({
    Title = "Select Boats",
	Values = ListSeaBoat}, function(Value)
        _G.SelectedBoat = Value
    end
})
_Sec_SeaEvent_42.CreateButton({
Title = "Buy Boats"}, function()
  replicated.Remotes.CommF_:InvokeServer("BuyBoat",_G.SelectedBoat)
end})
Q = _Dropdowns["Dropdown_Select_Sea_Level"] = {idx=1, vals={}}; _Sec_SeaEvent_42.CreateButton({
Title = "Select Sea Level",
Values = ListSeaZone}, function(Value)
  _G.DangerSc = Value
end})
Q = _Sec_SeaEvent_42.CreateToggle({
...
_Sec_SeaEvent_39 = PageSeaEvent.CreateSection("Entity Sea Event")
...
_Sec_SeaEvent_40 = PageSeaEvent.CreateSection("Kitsune Island / Event")
local Check_Kitsu = MakeLabel(_Sec_SeaEvent_42, "Kitsune Island Status", "")
spawn(function()
    while wait(0.2) do
...
_Sec_SeaEvent_42.CreateButton({
Title = "Trade Items Azure"}, function()
  replicated.Modules.Net:FindFirstChild("RF/KitsuneStatuePray"):InvokeServer()
end})

_Sec_SeaEvent_42.CreateButton({
Title = "Talk with kitsune statue"}, function()
  replicated.Modules.Net:FindFirstChild("RE/TouchKitsuneStatue"):FireServer()
end})

_Sec_SeaEvent_41 = PageSeaEvent.CreateSection("Frozen Dimension Event")

local FloD = MakeLabel(_Sec_SeaEvent_42, "FrozenDimension Status", "")
spawn(function()
    pcall(function()
        while wait(0.2) do
...
local SPYING = MakeLabel(_Sec_SeaEvent_42, "Spy Status", "")
spawn(function()
    while wait(0.2) do
        pcall(function()
...
_Sec_SeaEvent_42 = PageSeaEvent.CreateSection("Multi Levi")
...
_Dropdowns["Dropdown_Multi_Levi_Players"] = {idx=1, vals={}}; _Sec_SeaEvent_42.CreateButton({
...
_Sec_SeaEvent_42.CreateButton({
    Title = "Refresh Player List"}, function()
        MultiLeviPlayerList = {}
        for _, v in pairs(game:GetService("Players"):GetPlayers()) do
            if v ~= plr then
                table.insert(MultiLeviPlayerList, v.Name)
...
_Sec_Esp_43 = PageEsp.CreateSection("Esp")
...
_Sec_Raids_44 = PageRaids.CreateSection("Fruits Options")
...
local stockParagraph = MakeLabel(_Sec_Raids_47, "Stock Fruit", "Loading...")
...
RandomFF = _Sec_Raids_47.CreateToggle({
...
DropF = _Sec_Raids_47.CreateToggle({
...
StoredF = _Sec_Raids_47.CreateToggle({
...
TwF = _Sec_Raids_47.CreateToggle({
...
BringF = _Sec_Raids_47.CreateToggle({
...
_Dropdowns["Dropdown_Select_Fruit_Shop"] = {idx=1, vals={}}; _Sec_Raids_47.CreateButton({
    Title = "Select Fruit Shop"}, function(Value)
        getgenv().SelectFruit = Value
    end
})
_Sec_Raids_47.CreateToggle({
...
_Sec_Raids_45 = PageRaids.CreateSection("Dungeon Event / Raiding")
DungeonTables = {"Flame","Ice","Quake","Light","Dark","String","Rumble","Magma","Human: Buddha","Sand","Bird: Phoenix","Dough"}
Q = _Dropdowns["Dropdown_Select_Chip"] = {idx=1, vals={}}; _Sec_Raids_47.CreateButton({
Title = "Select Chip",
Values = DungeonTables}, function(Value)
  _G.SelectChip = Value
end})
Q = _Sec_Raids_47.CreateToggle({
...
AutoChipBeli = _Sec_Raids_47.CreateToggle({
...
AutoChipFruit = _Sec_Raids_47.CreateToggle({
...
StartR = _Sec_Raids_47.CreateToggle({
...
Raiding = _Sec_Raids_47.CreateToggle({
...
_Sec_Raids_46 = PageRaids.CreateSection("Items Law/Order Sword")

_Sec_Raids_47.CreateButton({
Title = "Buy Microchip Law"}, function()
  replicated.Remotes.CommF_:InvokeServer("BlackbeardReward","Microchip","2")
end})
_Sec_Raids_47.CreateButton({
Title = "Start Law Raids"}, function()
...
_Sec_Raids_47 = PageRaids.CreateSection("Raids Dungeons")
...
_Sec_Combat_48 = PageCombat.CreateSection("Combat / AimBot")

local __indexPlayer = MakeLabel(_Sec_Combat_50, "All Players On Server", "")
...
local __AimBotTurn = MakeLabel(_Sec_Combat_50, "Aimbot Status", "")
...
_Dropdowns["Dropdown_Select_Players"] = {idx=1, vals={}}; _Sec_Combat_50.CreateButton({
    Title = "Select Players",
    Values = PlrList}, function(Value)
        _G.PlayersList = Value
    end
})

_Sec_Combat_50.CreateToggle({
...
_Dropdowns["Dropdown_Select_Aim_Method"] = {idx=1, vals={}}; _Sec_Combat_50.CreateButton({
    Title = "Select Aim Method"}, function(Value)
        ABmethod = Value
    end
})
...
_Sec_Combat_49 = PageCombat.CreateSection("Quests Players")

_Sec_Combat_50.CreateButton({
    Title = "Get player quests"}, function()
        pcall(function()
...
_Sec_Combat_50 = PageCombat.CreateSection("LocalPlayer Settings")
...
_Sec_Travel_51 = PageTravel.CreateSection("Travel - Worlds")

_Sec_Travel_54.CreateButton({
Title = "Travel East Blue (World 1)"}, function()
  replicated.Remotes.CommF_:InvokeServer("TravelMain")
end})
_Sec_Travel_54.CreateButton({
Title = "Travel Dressrosa (World 2)"}, function()
  replicated.Remotes.CommF_:InvokeServer("TravelDressrosa")
end})
_Sec_Travel_54.CreateButton({
Title = "Travel Zou (World 3)"}, function()
  replicated.Remotes.CommF_:InvokeServer("TravelZou")
end})
_Sec_Travel_52 = PageTravel.CreateSection("Travel - Island")
Location = {}
for i,v in pairs(workspace["_WorldOrigin"].Locations:GetChildren()) do  
  table.insert(Location ,v.Name)
end
Travelllll = _Dropdowns["Dropdown_Select_Travelling"] = {idx=1, vals={}}; _Sec_Travel_54.CreateButton({
Title = "Select Travelling",
Values = Location}, function(Value)
  _G.Island = Value
end})
GoIsland = _Sec_Travel_54.CreateToggle({
Title = "Auto Travel", 
Default = false}, function(Value)
  _G.Teleport = Value
  if Value then
...
_Sec_Travel_53 = PageTravel.CreateSection("Travel - Portal")
if World1 then
  Location_Portal = {
    "Sky",
    "UnderWater"
...
PortalTP = _Dropdowns["Dropdown_Select_Portal"] = {idx=1, vals={}}; _Sec_Travel_54.CreateButton({
Title = "Select Portal",
Values = Location_Portal}, function(Value)
  _G.Island_PT = Value
end})
_Sec_Travel_54.CreateButton({
Title = "requestEntrance"}, function()
...
_Sec_Travel_54 = PageTravel.CreateSection("Travel - NPCs")
for _, v in pairs(replicated.NPCs:GetChildren()) do table.insert(NPCList, v.Name)end
NPCsPos = _Dropdowns["Dropdown_Select_NPCs"] = {idx=1, vals={}}; _Sec_Travel_54.CreateButton({
Title = "Select NPCs",
Values = NPCList}, function(Value)
  NPClist = Value
end})
GoNPCs = _Sec_Travel_54.CreateToggle({
...
_Sec_Misc_60 = PageMisc.CreateSection("Server - Function")
_Sec_Misc_63.CreateButton({
    Title = "Redeem All Codes"}, function()
        local codes = {
...
_Sec_Misc_63.CreateButton({
Title = "Rejoin Server"}, function()
  game:GetService("TeleportService"):Teleport(game.PlaceId, game.Players.LocalPlayer)
end})
_Sec_Misc_63.CreateButton({
    Title = "Hop Server"}, function()
        task.spawn(function()
            local HttpService = game:GetService("HttpService")
            local TeleportService = game:GetService("TeleportService")
            local PlaceId = game.PlaceId
...
_Sec_Misc_63.CreateButton({
Title = "Hop to Lowest Players"}, function()
  local Http = game:GetService("HttpService")
  local TPS = game:GetService("TeleportService")
  local Api = "https://games.roblox.com/v1/games/"
...
_Sec_Misc_63.CreateButton({
Title = "Hop to Lowest Pings Server"}, function()
local HTTPService = game:GetService("HttpService")
local TeleportService = game:GetService("TeleportService")
...
_Sec_Misc_63.CreateButton({
    Title = "Input Job Id"}, function(Value)
        getgenv().Job = Value
    end
})

_Sec_Misc_63.CreateButton({
    Title = "Teleport [Job ID]"}, function()
...
_Sec_Misc_63.CreateButton({
Title = "Copy JobID"}, function()
  setclipboard(tostring(game.JobId))
end})

_Sec_Misc_61 = PageMisc.CreateSection("Player Gui / Others")

_Sec_Misc_63.CreateButton({
Title = "Open Awakenings Expert"}, function()
  plr.PlayerGui.Main.AwakeningToggler.Visible = true
end})
_Sec_Misc_63.CreateButton({
Title = "Open Title Selection"}, function()
  replicated.Remotes.CommF_:InvokeServer("getTitles",true)
  plr.PlayerGui.Main.Titles.Visible = true
end})
DisbleChat = _Sec_Misc_63.CreateToggle({
...
DisbleLeaderB = _Sec_Misc_63.CreateToggle({
...
_Sec_Misc_63.CreateButton({
Title = "Set Pirate Team"}, function()
  Pirates()
end})  
_Sec_Misc_63.CreateButton({
Title = "Set Marine Team"}, function()
  Marines()
end})
UnPortal = _Sec_Misc_63.CreateToggle({
...
_Sec_Misc_62 = PageMisc.CreateSection("Graphics / Haki Stats")

HakiSt = {"State 0","State 1","State 2","State 3","State 4","State 5"}
HakiStat = _Dropdowns["Dropdown_Select_Haki_States"] = {idx=1, vals={}}; _Sec_Misc_63.CreateButton({
Title = "Select Haki States",
Values = HakiSt}, function(Value)
  _G.SelectStateHaki = Value
end})
_Sec_Misc_63.CreateButton({
Title = "ChangeBusoStage"}, function()
...
rtxM = _Sec_Misc_63.CreateToggle({
...
_Sec_Misc_63 = PageMisc.CreateSection("Configure - God")
_Sec_Misc_63.CreateButton({
Title = "Rain Fruits (Client)"}, function()
...
briggt1 = _Sec_Misc_63.CreateToggle({
...
DayN = _Dropdowns["Dropdown_Select_Time"] = {idx=1, vals={}}; _Sec_Misc_63.CreateButton({
Title = "Select Time"
Default = Day}, function(Value)
  _G.SelectDN = Value
end})
dayornight = _Sec_Misc_63.CreateToggle({
Title = "Turn on Time", 
Default = false}, function(Value)
  _G.daylightN = Value
end})
...
walkWater = _Sec_Misc_63.CreateToggle({
...
iceWalk = _Sec_Misc_63.CreateToggle({
...
function AttackNoCoolDown()
    ...
    local successFlags, combatRemoteThread = pcall(function()
        local ok, v = pcall(function() return require(modules.Flags).COMBAT_REMOTE_THREAD end); return ok and v or false
    end)
...
local function DisableCameraShake()
    local cameraModule = nil; pcall(function() cameraModule = require(ReplicatedStorage.Util.CameraShaker) end)
    cameraModule:Stop()
end
...
-- HOP BOSS TAB
-- =============================================
local _Sec_Boss_1 = PageBoss.CreateSection("World 3 Bosses")

-- Helper: hop to a server where the boss exists
local function HopBoss(bossName, bossPos)
    local Http = game:GetService("HttpService")
    local TPS = game:GetService("TeleportService")
    -- First teleport to known spawn location
    if bossPos then
        pcall(function() _tp(bossPos) end)
        task.wait(1.5)
    end
    -- Check if boss is already here
    local found = workspace.Enemies:FindFirstChild(bossName)
        or game:GetService("ReplicatedStorage"):FindFirstChild(bossName)
...
local function HopUntilBoss(bossName, bossPos, flagKey)
    if _G[flagKey] then return end -- already running
    _G[flagKey] = true
    task.spawn(function()
        while _G[flagKey] do
            pcall(function()
                ...
                if bossPos then _tp(bossPos) task.wait(1) end
                -- Hop
                local Http = game:GetService("HttpService")
                local TPS = game:GetService("TeleportService")
                local req = game:HttpGet("https://games.roblox.com/v1/games/"..game.PlaceId.."/servers/Public?sortOrder=Asc&limit=100")
...
-- Rip Indra
_Sec_Boss_1.CreateButton({Title = "Hop Rip Indra"}, function()
    HopBoss("rip_indra True Form", CFrame.new(-5344, 423, -2725))
end)
_Sec_Boss_1.CreateButton({Title = "Auto Hop Rip Indra (Until Found)"}, function()
    HopUntilBoss("rip_indra True Form", CFrame.new(-5344, 423, -2725), "HopRipIndra")
end)

-- Dough King
_Sec_Boss_1.CreateButton({Title = "Hop Dough King"}, function()
    HopBoss("Dough King", CFrame.new(-1943, 251, -12337))
end)
_Sec_Boss_1.CreateButton({Title = "Auto Hop Dough King (Until Found)"}, function()
    HopUntilBoss("Dough King", CFrame.new(-1943, 251, -12337), "HopDoughKing")
end)

-- Cake Prince
_Sec_Boss_1.CreateButton({Title = "Hop Cake Prince"}, function()
    HopBoss("Cake Prince", CFrame.new(-2151, 149, -12404))
end)
_Sec_Boss_1.CreateButton({Title = "Auto Hop Cake Prince (Until Found)"}, function()
    HopUntilBoss("Cake Prince", CFrame.new(-2151, 149, -12404), "HopCakePrince")
end)

-- Cake Queen
_Sec_Boss_1.CreateButton({Title = "Hop Cake Queen"}, function()
    HopBoss("Cake Queen", CFrame.new(-678, 381, -11114))
end)

-- Soul Reaper
_Sec_Boss_1.CreateButton({Title = "Hop Soul Reaper"}, function()
    HopBoss("Soul Reaper", CFrame.new(-9524, 315, 6655))
end)

-- Longma
_Sec_Boss_1.CreateButton({Title = "Hop Longma"}, function()
    HopBoss("Longma", CFrame.new(-10238, 389, -9549))
end)

-- Tyrant of the Skies
_Sec_Boss_1.CreateButton({Title = "Hop Tyrant of the Skies"}, function()
    HopBoss("Tyrant", CFrame.new(-13244, 2270, -7200))
end)

-- Kilo Admiral (Sea 3)
_Sec_Boss_1.CreateButton({Title = "Hop Kilo Admiral"}, function()
    HopBoss("Kilo Admiral", CFrame.new(2764, 432, -7144))
end)

-- Captain Elephant
_Sec_Boss_1.CreateButton({Title = "Hop Captain Elephant"}, function()
    HopBoss("Captain Elephant", CFrame.new(-13376, 433, -8071))
end)

-- Stone
_Sec_Boss_1.CreateButton({Title = "Hop Stone"}, function()
    HopBoss("Stone", CFrame.new(-1027, 92, 6578))
end)

-- Hydra Leader
_Sec_Boss_1.CreateButton({Title = "Hop Hydra Leader"}, function()
    HopBoss("Hydra Leader", CFrame.new(5821, 1019, -73))
end)

local _Sec_Boss_2 = PageBoss.CreateSection("World 1 & 2 Bosses")

-- Greybeard
_Sec_Boss_2.CreateButton({Title = "Hop Greybeard"}, function()
    HopBoss("Greybeard", CFrame.new(-5081, 85, 4257))
end)

-- Darkbeard
_Sec_Boss_2.CreateButton({Title = "Hop Darkbeard"}, function()
    HopBoss("Darkbeard", CFrame.new(3677, 62, -3144))
end)

-- Order (Sea 2)
_Sec_Boss_2.CreateButton({Title = "Hop Order"}, function()
    HopBoss("Order", CFrame.new(-6217, 28, -5053))
end)

-- Tide Keeper
_Sec_Boss_2.CreateButton({Title = "Hop Tide Keeper"}, function()
    HopBoss("Tide Keeper", CFrame.new(-3795, 105, -11421))
end)

-- Awakened Ice Admiral
_Sec_Boss_2.CreateButton({Title = "Hop Awakened Ice Admiral"}, function()
    HopBoss("Awakened Ice Admiral", CFrame.new(6403, 340, -6894))
end)

-- Don Swan
_Sec_Boss_2.CreateButton({Title = "Hop Don Swan"}, function()
    HopBoss("Don Swan", CFrame.new(2286, 15, 863))
end)

-- Cursed Captain
_Sec_Boss_2.CreateButton({Title = "Hop Cursed Captain"}, function()
    HopBoss("Cursed Captain", CFrame.new(916, 181, 33422))
end)

local _Sec_Boss_3 = PageBoss.CreateSection("Moon & Legendary Sword Hop")

-- Full Moon hop
_Sec_Boss_3.CreateButton({Title = "Hop Full Moon (Auto Hop Until Full Moon)"}, function()
    if _G.HopFullMoon then _G.HopFullMoon = false return end
    _G.HopFullMoon = true
    task.spawn(function()
        local Http = game:GetService("HttpService")
        local TPS = game:GetService("TeleportService")
        while _G.HopFullMoon do
            pcall(function()
                ...
                if moonId == "http://www.roblox.com/asset/?id=9709149431" then
                    _G.HopFullMoon = false -- Full moon found
                    return
                end
                local req = game:HttpGet("https://games.roblox.com/v1/games/"..game.PlaceId.."/servers/Public?sortOrder=Asc&limit=100")
                local data = Http:JSONDecode(req)
...
-- Near Full Moon hop (4/5 or 5/5)
_Sec_Boss_3.CreateButton({Title = "Hop Near Full Moon (4/5 or 5/5)"}, function()
    if _G.HopNearMoon then _G.HopNearMoon = false return end
    _G.HopNearMoon = true
    task.spawn(function()
        local Http = game:GetService("HttpService")
        local TPS = game:GetService("TeleportService")
        local nearMoonIds = {
            "http://www.roblox.com/asset/?id=9709149431", -- 5/5
            "http://www.roblox.com/asset/?id=9709149052", -- 4/5
        }
        while _G.HopNearMoon do
            pcall(function()
                local moonId = game:GetService("Lighting").Sky.MoonTextureId
                for _, id in ipairs(nearMoonIds) do
                    if moonId == id then
...
-- Legendary Sword hop
_Sec_Boss_3.CreateButton({Title = "Hop Legendary Sword (Any)"}, function()
    if _G.HopLegSword then _G.HopLegSword = false return end
    _G.HopLegSword = true
    task.spawn(function()
        local Http = game:GetService("HttpService")
        local TPS = game:GetService("TeleportService")
        local CommF = game:GetService("ReplicatedStorage").Remotes.CommF_
        while _G.HopLegSword do
            pcall(function()
                local found = CommF:InvokeServer("LegendarySwordDealer","1")
                    or CommF:InvokeServer("LegendarySwordDealer","2")
                    or CommF:InvokeServer("LegendarySwordDealer","3")
                if found then
                    _G.HopLegSword = false
                    return
                end
                local req = game:HttpGet("https://games.roblox.com/v1/games/"..game.PlaceId.."/servers/Public?sortOrder=Asc&limit=100")
                local data = Http:JSONDecode(req)
...
-- Shisui specifically
_Sec_Boss_3.CreateButton({Title = "Hop Shisui Sword"}, function()
    if _G.HopShisui then _G.HopShisui = false return end
    _G.HopShisui = true
    task.spawn(function()
        local Http = game:GetService("HttpService")
        local TPS = game:GetService("TeleportService")
        local CommF = game:GetService("ReplicatedStorage").Remotes.CommF_
        while _G.HopShisui do
            pcall(function()
                if CommF:InvokeServer("LegendarySwordDealer","1") then
                    _G.HopShisui = false return
                end
                local req = game:HttpGet("https://games.roblox.com/v1/games/"..game.PlaceId.."/servers/Public?sortOrder=Asc&limit=100")
                local data = Http:JSONDecode(req)
                for _, v in pairs(data.data) do
...
local _Sec_Boss_4 = PageBoss.CreateSection("Stop All Hops")
_Sec_Boss_4.CreateButton({Title = "Stop All Boss Hops"}, function()
    _G.HopRipIndra = false
    _G.HopDoughKing = false
    _G.HopCakePrince = false
    ...
    -- Paint all existing descendants
    for _, obj in ipairs(gui:GetDescendants()) do
        paintGui(obj)
    end

    -- Also patch newly added descendants (e.g. when tabs are opened)
    gui.DescendantAdded:Connect(function(obj)
        task.wait() -- let it initialize first
        paintGui(obj)
    end)
end)
