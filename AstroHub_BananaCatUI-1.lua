...
do
  ply = game.Players
  plr = ply.LocalPlayer
  -- Wait for character to be ready before grabbing Root
  if not plr.Character or not plr.Character:FindFirstChild("HumanoidRootPart") then
    plr.CharacterAdded:Wait()
    task.wait(0.5)
  end
  Root = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") or nil
  replicated = game:GetService("ReplicatedStorage")
  Lv = 0; pcall(function() Lv = game.Players.LocalPlayer.Data.Level.Value end)
  ...
  Energy = pcall(function() return plr.Character.Energy.Value end) and plr.Character:FindFirstChild("Energy") and plr.Character.Energy.Value or 0
  BringConnections = {}
  BossList = {}
  MaterialList = {}
  NPCList = {}
...
-- Safe load wait: skip if game already loaded or Loading gui already gone
pcall(function()
    local main = plr.PlayerGui:FindFirstChild("Main")
    if main and main:FindFirstChild("Loading") then
        repeat task.wait(0.1) until
            not plr.PlayerGui:FindFirstChild("Main")
            or not plr.PlayerGui.Main:FindFirstChild("Loading")
            or game:IsLoaded()
    end
end)
...
local Remotes = {}
pcall(function()
    Remotes.RFJobsRemoteFunction = replicated.Modules.Net["RF/JobsRemoteFunction"]
    Remotes.RFCraft = replicated:WaitForChild("Modules", 5):WaitForChild("Net", 5):WaitForChild("RF/Craft", 5)
end)
...
-- =============================================
-- DARK MARINE BLUE COLOR PATCHER
-- =============================================
-- Applied after the GUI loads (Nousigi Hub GUI in CoreGui)
task.spawn(function()
    -- Wait for the GUI to exist
    local timeout = tick() + 15
    repeat task.wait(0.2) until game.CoreGui:FindFirstChild("Nousigi Hub GUI") or tick() > timeout

    local gui = game.CoreGui:FindFirstChild("Nousigi Hub GUI")
    if not gui then return end

    -- Dark marine blue = RGB(0, 36, 85) / pure dark navy
    local DARK_BLUE   = Color3.fromRGB(0,  36,  85)   -- main dark blue
    local MED_BLUE    = Color3.fromRGB(0,  55, 120)   -- slightly lighter for buttons
    local ACCENT_BLUE = Color3.fromRGB(0,  80, 160)   -- toggle ON / active accent
    local TEXT_WHITE  = Color3.fromRGB(255,255,255)

    local function paintGui(obj)
        pcall(function()
            local name = obj.Name:lower()
            local cls  = obj.ClassName

            -- Title bar / top panel
            if name:find("topmain") or name:find("topbar") or name:find("header") then
            ...
            if name == "main" or name == "maincontainer" or name == "background" or name == "bg" then
                if obj:IsA("Frame") or obj:IsA("ScrollingFrame") then
                    obj.BackgroundColor3 = DARK_BLUE
                end
            end

            -- Tab side bar lines / tab buttons (were yellow)
            if name:find("tab") or name:find("sidebar") or name:find("pagebar") or name:find("navframe") then
                if obj:IsA("Frame") then
                    obj.BackgroundColor3 = DARK_BLUE
                end
                -- The selection indicator line on left tabs
                if obj:IsA("Frame") and obj.Size.X.Scale == 0 and obj.Size.X.Offset <= 6 then
                    obj.BackgroundColor3 = ACCENT_BLUE
                end
            end

            -- Tab text / page buttons
            if name:find("textbutton") or name:find("tabbutton") or name:find("pagebtn") then
                if obj:IsA("TextButton") or obj:IsA("ImageButton") then
                    obj.BackgroundColor3 = DARK_BLUE
                end
            end

            -- Toggle frames / toggle backgrounds
            if name:find("toggle") then
                if obj:IsA("Frame") or obj:IsA("ImageLabel") then
                    -- OFF state background stays dark, ON accent set via BackgroundColor3
                    obj.BackgroundColor3 = DARK_BLUE
                end
                if obj:IsA("TextButton") or obj:IsA("ImageButton") then
                    obj.BackgroundColor3 = MED_BLUE
                end
            end

            -- Click buttons
            if name:find("button") and not name:find("close") and not name:find("min") then
                if obj:IsA("TextButton") or obj:IsA("ImageButton") then
                    obj.BackgroundColor3 = MED_BLUE
                end
                if obj:IsA("Frame") then
            ...
            -- Section lines / dividers
            if name:find("section") or name:find("divider") or name:find("line") then
                if obj:IsA("Frame") and obj.Size.Y.Offset <= 3 then
                    obj.BackgroundColor3 = ACCENT_BLUE
                end
            end

            -- Yellow accent elements (the left tab indicator lines)
            if cls == "Frame" or cls == "ImageLabel" then
                if obj.BackgroundColor3 == Color3.fromRGB(255,200,0)
                or obj.BackgroundColor3 == Color3.fromRGB(255,165,0)
                or obj.BackgroundColor3 == Color3.fromRGB(255,215,0)
                or obj.BackgroundColor3 == Color3.fromRGB(255,170,0)
                or obj.BackgroundColor3 == Color3.fromRGB(250,220,50) then
                    obj.BackgroundColor3 = ACCENT_BLUE
                end
            end
        end)
